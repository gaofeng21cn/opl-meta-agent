...
- MAS evidence takeover 入口是 `npm run mas:evidence -- --mas-repo <mas-repo> --output-dir <dir> --opl-bin <opl> [--ai-reviewer-evaluation <reviewer-eval.json>]`：读取 MAS production acceptance、Agent Lab handoff、generated-surface handoff 和 owner-receipt 合同，生成 `suite_kind=mas_production_evidence_suite` 的 `mas-agent-lab-suite.json`，运行 `opl agent-lab run --suite`，再输出 refs-only developer work order、MAS capability candidate、mechanism patch proposal，或在缺 reviewer evaluation 时输出 typed blocker。
