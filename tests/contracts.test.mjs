...
test('meta-agent descriptor keeps OPL runtime authority outside the repo', () => {
  const descriptor = readJson('contracts/domain_descriptor.json');

  assert.equal(descriptor.domain_id, 'meta-agent');
  assert.equal(descriptor.domain_label, 'Meta Agent');
  ...
  assert.equal(descriptor.authority_boundary.meta_agent_can_train_or_deploy_model_weights, false);
  assert.equal(descriptor.authority_boundary.meta_agent_can_promote_default_agent_without_gate, false);
});

test('meta-agent stage plan covers research, build, eval, optimization, delivery, and learning', () => {
  ...
  assert.equal(stageControl.authority_boundary.meta_agent_owns_agent_building_semantics, true);
...
test('action catalog and owner receipts forbid target-domain authority writes', () => {
  ...
  assert.ok(ownerReceipt.forbidden_claims.includes('meta_agent_wrote_target_domain_truth'));
  assert.ok(ownerReceipt.forbidden_claims.includes('meta_agent_promoted_default_agent_without_gate'));
});
