...
function assertNotReadyClaimed(surface: JsonObject, label: string): void {
  [
    'target_agent_ready_claimed',
    'domain_ready_claimed',
    'production_ready_claimed',
  ].forEach((field) => {
    if (field in surface) assert.equal(surface[field], false, `${label}.${field}`);
  });
}
...
test('target-agent owner-chain evidence accepts live-progress refs without target authority claims', () => {
  ...
  assert.equal(canary.target_agent_owner_chain_evidence_ref, 'contracts/target_agent_owner_chain_evidence.json');
  assert.equal(
  ...
  [
    evidence.source_refs.action_catalog_ref,
    evidence.source_refs.stage_run_kernel_profile_ref,
    evidence.source_refs.stage_run_canary_evidence_ref,
    evidence.source_refs.real_target_agent_scaleout_evidence_ref,
    evidence.source_refs.production_acceptance_ref,
  ].forEach((ref) => assertRepoRefExists(ref as string));
  ...
  assert.equal(humanGateClosure.surface_kind, 'opl_meta_agent_stage_replay_human_gate_owner_chain_closure');
  assert.equal(humanGateClosure.closure_status, 'blocked_by_domain_owned_typed_blocker_ref');
  assert.equal((humanGateClosure.target_identity as JsonObject).domain_id, 'opl-meta-agent');
  ...
  assertNotReadyClaimed(humanGateClosure, 'humanGateClosure');
  assert.equal((humanGateClosure.authority_boundary as JsonObject).can_write_owner_receipt, false);
  assert.equal((humanGateClosure.authority_boundary as JsonObject).can_close_replay_success_path, false);
  ...
  assertNotReadyClaimed(liveProgressSummary, 'liveProgressSummary');
  ...
  assert.equal(productionLiveProgressSummary.opl_consumption_status, liveProgressSummary.opl_consumption_status);
  assert.equal(ownerTailClosure.closed_as_success, false);
  assert.equal(ownerTailClosure.success_claimed, false);
  assertNotReadyClaimed(ownerTailClosure, 'ownerTailClosure');
  ...
  assert.equal((ownerTailClosure.authority_boundary as JsonObject).refs_only, true);
  assert.equal((ownerTailClosure.authority_boundary as JsonObject).can_write_target_domain_truth, false);
  assert.equal((ownerTailClosure.authority_boundary as JsonObject).can_write_target_owner_receipt_body, false);
  assert.equal((ownerTailClosure.authority_boundary as JsonObject).can_claim_production_ready, false);
...
test('OMA domain-owner-chain scaleout exposes typed blocker refs without target authority claims', () => {
  ...
  assertNotReadyClaimed(scaleout, 'scaleout');
  ...
  [
    ...
    'can_claim_production_ready',
    'can_promote_default_agent_without_gate',
  ].forEach((field) => assert.equal(boundary[field], false, `scaleout.${field}`));
});
