...
test('OMA pack declares advisory cognitive-kernel contracts', () => {
  ...
  assert.equal(pack.tool_affordance_boundary.catalog_role, 'available_affordance_catalog_not_workflow_script');
  ...
  assert.equal(adoption.domain_id, 'opl-meta-agent');
...
test('OMA stage control plane declares tool boundaries and independent gates', () => {
  ...
  for (const stage of plane.stages) {
    ...
    assert.equal(stage.independent_gate_policy.gate_owner, 'opl-meta-agent');
    ...
    assert.equal(stage.independent_gate_policy.generated_surface_readiness_can_claim_quality_or_export, false);
  }
});
