...
- `agent/` is the real domain pack entry for this repo. `knowledge/`, `prompts/`, `quality_gates/`, `skills/`, and `stages/` must expose resolvable, non-empty, placeholder-free domain-owned files, and `contracts/stage_control_plane.json` `prompt_refs` must resolve to those files.
...
The test suite verifies contract fields, the OPL-generated interface bundle, real `agent/` domain pack files, `stage_control_plane.prompt_refs` path resolution, non-empty files, and placeholder absence.
