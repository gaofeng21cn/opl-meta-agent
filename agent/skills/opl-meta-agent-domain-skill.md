...
2. 执行 `web-experience-research`、用户提供参考设计读取，或消费 OPL source ingest / Codex extraction 形成的 pattern packet refs；只吸收可迁移模式，不复制外部 runtime、私有数据或领域 truth。
...
- 用户提供论文、PDF、repo 或案例系统作为参考时，必须保留 reference design refs / pattern packet refs，并说明其只作为架构/评估/工作流模式来源。
