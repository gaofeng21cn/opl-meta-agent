...
当 OMA 已生成或接收到 `opl_meta_agent_developer_patch_work_order`，并且需要进入实际执行阶段时，使用本 skill 把 work order 委托给 OPL Framework 的 `work-order execute` primitive。
...
- `opl_work_order_execution_result_ref`
...
- passthrough args 只能追加给 OPL work-order primitive，不能在 OMA 内解释成 runner、queue、worktree 或 absorb 行为。
...
- 禁止在 OMA 内直接调用 target owner closeout hook；hook 调用只能由 OPL Framework 在 absorb 后转交 target-domain owner action。
