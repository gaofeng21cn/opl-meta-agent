...
- Design stage admission as progress-first. Validator, schema, and materializer feedback may repair projection, ref, stage input, `requires`, or expected receipt drift inside the same stage when the correction is mechanically derivable from valid design objects and authority refs; it must not become a terminal blocker, a new top-level stage, or a private control plane. Semantic, source-evidence, reviewer, non-derivable design, zero-output, and corrupt-output gaps become quality debt plus a consumable diagnostic or route-back and block accepted/delivery/promotion/ready claims only. Explicit owner/human decisions, forbidden target-truth writes, unavailable executors, irreversible actions, and safety/permission/authority/identity/currentness boundaries still route to typed blocker or human gate.
...
Keep target-pack admission/materialization claims closed when source bytes are unavailable, the extraction receipt cannot be bound to them, required semantic artifacts are empty, or source anchors do not support a claimed workflow step. Preserve any readable raw source, note, partial packet, extraction finding, or design fragment as progress with quality debt so Codex can continue, repeat, skip, or route back; do not hide gaps behind a default stage graph, generic pattern note, or fixed packet template. Hard-stop only when bytes are unreadable, semantic refs escape the packet directory, authority/non-claim fields are invalid, or identity/currentness/safety/permission requires it.
