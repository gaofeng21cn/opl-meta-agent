...
3. Include allowed and forbidden write sets, expected verification, rollback/version refs, owner route, no-forbidden-write proof, post-absorb handoff expectation, and target runtime/read-model consumption checks.
4. Choose the work-order lane before writing prose: `stage-route` for stage/owner sequence defects, `specialist-skill` for missing professional method, `tool-connector` for helper/integration capability gaps, `quality-gate` for false pass/fail or weak reviewer gates, `read-model-currentness` for stale projections, and `authority-boundary` for forbidden owner claims.
5. Treat trajectory learning as advisory input only. It can justify a proposal, route-back, or bounded work order, but cannot auto-promote defaults, mutate runtime behavior, or bypass owner acceptance.
