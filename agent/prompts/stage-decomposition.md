...
- `selected_opl_profile_refs`、`profile_selection_rationale`、`profile_requirement_refs` 和 profile requirements；它们必须来自 OPL `profiles select/inspect` readback。
...
3. 把 selected OPL profile requirements 映射进 stage graph：required stage archetypes 进入 stage_contract requires，reference pack / source freshness / provenance requirements 进入 knowledge refs，tool connector requirements 进入 tool refs，quality/evaluation requirements 进入 quality gate。
...
- `target_agent`：`domain_id`、`domain_label`、`delivery_domain`、`target_brief`、`selected_opl_profile_refs`、`profile_selection_rationale`。
- `stage_control_plane` 和每个 stage 必须保留 `selected_profile_refs`、`profile_selection_receipt_ref` 和 `profile_requirements`；stage inputs/requires 必须引用 profile selection receipt。
