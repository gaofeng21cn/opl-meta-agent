...
在另一个新 thread 中，消费 prior findings 与 repair map，用同一 source/rubric 对 exact changed artifact refs/hash 逐条关闭 accepted findings。返回 `verdict`、整体 `evidence_refs`、remaining quality-debt refs，并为每个 accepted `finding_id` 输出 `closed`、`partially_closed` 或 `still_open`。只有 `required_finding_not_closed`、`repair_regression` 或 `critical_new_finding` 可以触发下一轮 Repair；普通新建议只能记录为 `optional_observation` 或 quality debt，不得重开循环。不得继承 repair rationale，也不得按 repairer 自述关闭 finding。`repair_required` re-reviewer 只可返回 `route_impact.stage_route_recommendation`；terminal re-reviewer 返回 `route_impact.stage_route_decision`，其 closeout 字段由 StageRunController 物化为正式 Review receipt。
