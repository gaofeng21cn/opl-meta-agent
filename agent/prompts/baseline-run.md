...
把 candidate Agent Pack、AgentBuildReceipt 和 independent reviewer evidence 组织成薄 `foundry_evaluation_request` 与 target-bound OPL Foundry Lab evaluation work order。OMA 不执行 suite，也不生成 suite plan、suite result、execution receipt、owner receipt、learning ledger 或 promotion result。

## 输入

- candidate Agent Pack ref、target descriptor ref 与 AgentBuildReceipt ref。
- thin baseline evaluation request、quality-gate、improvement-candidate 与 promotion-gate refs。
- independent AI reviewer evaluation 与 artifact morphology evidence refs。
- OPL Foundry Lab evaluation work-order contract。

## 步骤

1. 确认 request 只携带 domain task intent 与 refs，不含 target identity、environment、probe、scorecard spec、completion policy、`observed_status`、`passed`、`gate_status` 或伪 result/receipt。
2. 校验 work order 中的 target identity 同时包含 `domain_id`、`target_agent_ref` 和 `descriptor_ref`，request/suite/task identity 与 target 一致。
...
5. 输出 evaluation request、Foundry evaluation work order 和 expected result ref；若没有 OPL 返回的 evaluation observations，保持 pending/typed-blocker route，不生成结果。
...
- request 不包含 target identity、framework-owned suite plan、observations、pass/fail verdict 或 hosted ledger body。
