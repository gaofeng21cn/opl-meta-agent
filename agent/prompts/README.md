...
Machine boundary: 本文是人读支撑索引。机器 truth 继续归 `agent/stages/manifest.json`、`contracts/pack_compiler_input.json`、本目录非 README prompt 文件和 tests；本文不作为 `prompt_refs` 或 `required_domain_pack_paths`。

本目录是 `agent/stages/manifest.json` 中 `prompt_ref` 的 domain-owned prompt body。OPL Framework 可以投影这些 locator，但不能复制、改写或替代本仓的 agent-building domain semantics。
