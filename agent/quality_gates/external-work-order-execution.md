...
用于判断 OMA developer patch work order 是否可以交给 OPL Framework `work-order execute` primitive 执行。
...
- OPL work-order execution result ref。
...
- OMA 调用形状为 `opl work-order execute --work-order <path> --json`。
- passthrough args 原样追加给 OPL work-order primitive。
- target worktree lifecycle、queue、attempt ledger、absorb、cleanup 和 execution receipt 均由 OPL Framework / target owner 控制面持有。
- post-absorb target owner closeout hook 只能由 OPL Framework 调用 target-domain owner action；OMA receipt 只能记录 `owner_closeout_hook_delegated=true`、`target_owner_closeout_owner=target-domain via OPL` 和 `oma_can_write_owner_receipt=false`。
