...
OMA 的标准形态固定为 `Standard Agent Pack + TypeScript agent-design/materialization helpers`。Agent 身份、stage、prompt、skill、knowledge、tool affordance、quality gate 和 authority refs 只由 `agent/` Markdown 与 `contracts/` JSON 定义；TypeScript 只是已声明 authority/helper 的实现语言，不是第二种 Agent 类型，也不得持有 generic runtime 或 generated surface。

OMA 生成或接管的任何目标 Agent 都必须采用同一语言中立形态：`Declarative Standard Agent Pack + optional domain helpers`。helper 可以按领域选择 Python 或 TypeScript，但语言不得改变 Agent identity、Pack ABI、golden path、generated CLI/App/runtime surface 或 owner boundary；没有必要 helper 时保持 pack-only。
...
- 不把 helper language、repo 主语言或 implementation root 写成 Agent 类型；标准 Agent membership 只由声明式 Pack 与 OPL admission/conformance 决定。
