...
- 文档职责与生命周期归 `docs/README.md`；修改合同、Stage 或公开入口时同步其唯一说明页。
- 已完成计划移除执行步骤，仅有持续决策价值的依据进入 `docs/history/`；旧接口不保留别名、兼容页或继续执行提示。
- Markdown 检查只验证链接、引用、结构化元数据与生成投影完整性；内容正确性依据当前合同、调用者与实际验收证据判断，不用固定措辞或覆盖台账代替。
