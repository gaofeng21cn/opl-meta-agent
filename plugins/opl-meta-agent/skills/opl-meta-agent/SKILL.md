...
使用 OPL Meta Agent 创建、接管测试、修复或持续改进 OPL-compatible Foundry Agent。OMA 读取用户自然语言、目标领域材料、用户提供的论文/资料/外部参考设计、已有 agent repo、Agent Lab evidence 和 reviewer evidence，产出 target-agent brief、stage pack、quality gates、AgentBuildReceipt、thin Foundry evaluation request、OPL Foundry Lab evaluation work order、developer patch work order、mechanism proposal、target capability candidate 或 typed blocker ref/shape。
...
- 用户提供 typed reference-design packet 时，每个声明 source 都应有 packet 覆盖；用户 packet 是 active design origin，seed 只作 context。覆盖或 disposition 缺口在已有可读 source/design artifact 时记为质量债并继续。每个 admitted workflow step 必须有可追溯 disposition，但只有独立开放判断才物化成独立 stage；同一判断内的机械步骤留在 stage 内，不得由 schema 或 validator 阻断推进。
- 若用户提供论文、PDF、GitHub repo、产品文档或案例系统，标准链路是 `source extraction -> OPL refs-only ReferenceDesignPatternPacket -> OMA ReferenceDesignPacket -> TransferMap -> AgentPackPlan -> DesignAdmissionReceipt -> target pack materialization -> AgentBuildReceipt`。raw source、短 note、opaque packet ref、缺 steps 或缺 anchors 仍是可消费设计输入：记录质量债务并由 Codex route-back 到 source extraction、intent 或 stage-decomposition；不得自动拼固定“已提炼”文案，也不得因 packet/schema 不完整阻断后续 stage 启动。
...
- OMA 设计 stage 大小时，默认由 stage 主提示词承担 top-level decomposition，由 repo-local professional skill 承担 stage 内专业方法判断。一个 top-level stage 只应承载一个主要开放语义判断；确定性生成、校验、文件物化、helper receipt 和 readback 留在 stage 内。若需要不同 owner、知识源、quality gate、handoff 或失败路由，就拆 stage；若只是同一判断的机械步骤，就合并或删除。刻意保留的大 stage 必须用 typed subpacket / gate 暴露内部边界，不能用 schema 或 validator 替代语义拆分判断。validator/schema/materializer 发现的可机械推导 format、projection、ref 或 expected receipt 缺口，应在同一 stage admission 内有界修正并继续推进；已有可消费设计/pack artifact 时，语义、source/evidence 或 reviewer 缺口记录质量债务与 route-back，阻止 accepted/delivery/promotion/ready 声明但继续 stage transition。只有零可消费 artifact、artifact 损坏、显式 owner/human 决策、安全/权限/authority/currentness 才 typed blocker / human gate。
- Codex CLI 是唯一 stage 语义控制面；DesignAdmissionReceipt、subpacket、schema、validator 和 materializer 只提供 findings。任何 raw/partial agent design 都可进入下一 stage；Codex 可根据 Agent Lab/ reviewer 负结果回到 intent、stage-decomposition、baseline 或 optimizer stage 重开。
...
- thin evaluation request / Foundry evaluation work-order refs，以及由 OPL 外部返回的 suite plan、baseline / takeover / external-suite result 与 execution-receipt refs；
...
缺任一项时记录 delivery quality debt 或 owner-gated next step，不声明 target agent delivered、domain ready、production ready、quality verdict、App live ready 或 default promotion；已有可消费设计/pack artifact 时仍允许后续 stage 启动。只有零可读 artifact 或真实安全、权限、authority、currentness、human gate 才返回硬 blocker。
...
- OMA 可以写 agent-building semantics、declarative pack、thin evaluation request、Foundry evaluation work-order candidate、developer work-order policy、target capability candidate、mechanism proposal 和 OMA-owned blocker shape/ref；不得代写 target-domain 或 OPL runtime blocker body。
...
完成声明必须绑定到 target-agent handoff evidence、Agent Lab / reviewer evidence、self-evolution closeout、owner route 和明确 closeout outcome。缺 owner authority 时关闭完成/ready/promotion 声明并输出质量债、human gate、route-back 或 owner-gated continuation；不得阻止普通 stage transition。
