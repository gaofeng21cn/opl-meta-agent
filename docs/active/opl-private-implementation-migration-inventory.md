...
- Large script-level policy: `agent-evidence-takeover.ts` and `improve-from-agent-lab-suite.ts` 仍承载 AI reviewer gate、capability mapping 和 proposal assembly；work-order closeout / runtime consumption / workspace proof / readiness completeness / generic refs intake repetition 已收薄到 helper。
