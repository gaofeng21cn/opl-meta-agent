...
2026-05-22 `codex/oma-bootstrap-domain-pack-split` 已把 `scripts/bootstrap-sample-agent.ts` 内的 sample / real-target domain-pack writer 拆到 `scripts/lib/bootstrap-domain-packs.ts`。入口脚本从 746 行降到 483 行，继续只负责 bootstrap orchestration、OPL scaffold / Agent Lab 调用、baseline receipt 和 scaleout ledger 落盘；新 helper 只物化 refs-only `action_catalog` 与 `stage_control_plane` 测试包，不声明 OPL standard scaffold/generator 或 Agent Lab primitive 已完成替代。
...
| `scripts/lib/work-order-policy.ts` | 717 | `already_thin_adapter` | imported by `agent:evidence` and `improve:external-suite` | 共享 target patch-loop closeout refs、runtime/read-model proof、workspace proof、forbidden target surface defaults、refs-only work-order completeness builder、developer patch work-order validator、generic refs intake / owner-route normalization / evidence refs extraction、`*_efficiency_handoff_projection` intake 和 OPL Agent Lab owned primitive refs builder | 是 repo-local developer work-order materializer helper，不运行 suite、不签 promotion、不持有 target truth；primitive refs 只指向 OPL Agent Lab owner；validator 只做 shape fail-closed，不授权 patch 或 promotion；efficiency handoff 只消费 refs-only projection，不新增 domain-specific command family | work-order policy constants、refs-only proof builders、target-agent evidence materializer helpers、shape validation | OPL Agent Lab/App/registry runtime consumption receipts、generic readiness primitive implementation 和 promotion gate | 保持 pure helper；稳定合同文本继续迁入 `agent/`、`contracts/` 或 authority refs | `npm test`、`npm run typecheck` |
...
- Large script-level policy: `agent-evidence-takeover.ts` 已把 proposal/blocker/work-order 输出组装收薄到 `scripts/lib/agent-evidence-materializer.ts`，入口仍承载 production evidence intake、suite generation、OPL Agent Lab invocation 和 artifact 落盘；`improve-from-agent-lab-suite.ts` 已把 target improvement mapping / patch traceability / editable surface inference 收薄到 helper；`bootstrap-sample-agent.ts` 已把 sample / real-target domain-pack writer 收薄到 `scripts/lib/bootstrap-domain-packs.ts`。后续仍需防止 materializer helper 回长成私有 promotion/workbench/scaffold generator。work-order closeout / runtime consumption / workspace proof / readiness completeness / generic refs intake repetition 已收薄到 helper。
...
4. `scripts/lib/bootstrap-domain-packs.ts` 已落地为 bootstrap smoke 的 refs-only pack writer helper，入口脚本不再私有承载 sample / real-target action/stage pack assembly。
5. Keep the scripts as materializers that consume helper policy and target refs.
6. Continue migrating stable AI reviewer gate / capability mapping / proposal policy into `agent/prompts/`, `agent/stages/`, `agent/quality_gates/`, `contracts/` or `runtime/authority_functions/`.
7. Do not add repo-local scheduler, queue, attempt ledger, status/workbench shell, registry owner, Agent Lab runner or promotion gate.
