...
| Stage-Native compiler refs | `done_as_ref_templates_with_split_tail` | `contracts/stage_control_plane.json`、`scripts/lib/stage-native-artifact-contract.ts`、`scripts/lib/stage-decomposition-pack-draft.ts`、`tests/stage-decomposition-materializer.test.ts`、`tests/contracts.test.ts` | 生成 target agent pack 时包含 physical kernel locator、conformance 和 workbench consumption refs-only templates；不创建 runtime state、不 owner promotion、不管理 target worktree、不写 owner receipt body。`stage-decomposition-pack-draft.ts` 已触及 1500 行拆分信号，下一轮优先拆成 builder / validator / materializer。 |
...
6. `script_to_pack_hygiene`
   持续把稳定 policy 迁入 declarative pack、contracts、cognitive-kernel refs 或 OPL primitive；developer work-order 默认 policy 与 standard Foundry policy bundle 已有 contract-backed projection，下一步是继续减少仍需保留的 materializer/helper 语义并拆分 stage-decomposition split-pressure。保留脚本必须能指向 `runtime/authority_functions`、smoke action、fixture/proof helper 或 developer work-order materializer。当前 machine gate 要求所有 `scripts/**/*.ts` 都同时完成 morphology classification 和 script-to-pack retirement/retention gate 覆盖；每次新增或收薄脚本，都要更新 `script_to_pack_retirement_gates`，并跑 `tests/source-purity.test.ts`。
