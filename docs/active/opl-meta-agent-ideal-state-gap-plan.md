...
- 2026-07-07 cleanup landing 结论：测试 support 不再 re-export Node 标准库，`external-suite-fixtures` 不再持有重复 repo/JSON helper；legacy professional skill 物理 `TOMBSTONE.md` 文件已合并为 `capability_map` / README redirect ledger；原 cleanup item `5 line-budget aliases` 已从 package scripts、`scripts/verify.sh` 和机器 policy/readback caller 面退役，canonical operator surface 只保留 `source-structure` / `source-structure:strict`。`6 execute-external-work-order.ts` 仍是 OMA developer-work-order validation aperture，OPL 只证明 executor primitive parity；`7 stage-decomposition-pack-draft-parts/**` 与 `work-order-*` helpers 仍缺 OPL Pack/Stage/WorkOrder full schema/materializer parity。该结论不声明 broader script cleanup complete，细节归私有实现迁移台账。
