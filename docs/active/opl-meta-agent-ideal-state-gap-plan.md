...
| Work-order and mechanism proposal materialization | `done_with_more_real_loop_tail` | `build-agent-baseline`、`takeover:test`、`improve:external-suite`、`agent:evidence`、`execute:external-work-order`、matching tests | 只能输出 candidate package、developer work order、target capability candidate、mechanism proposal 或 typed blocker；`build-agent-baseline` default path 缺 target source morphology、owner route、generated surface consumption、private residue decision、accepted owner-answer shape、StageRun refs-only boundary 或 no-forbidden-write proof 时必须 fail closed，不启动半标准 target agent；target truth、artifact、memory、quality verdict 和 owner receipt body 仍归 target owner。 |
...
| Target-agent handoff vocabulary | Default-path gate 已把 target-agent generic vocabulary、developer work-order shape、source morphology、owner route、generated surface consumption、private residue decision、accepted owner-answer shape 和 no-forbidden-write proof 纳入 fail-closed readback；App drilldown、repeat cohort、real source patch-loop、reviewer direct evidence 和 target owner receipt 样本走后置 evidence lane。 | `contracts/action_catalog.json#actions/build-agent-baseline/new_agent_delivery_gate`、`scripts/build-agent-baseline.ts#assertNewAgentDeliveryGate`、production acceptance contracts、OPL read models、target owner receipts / typed blockers、`docs/status.md` |
