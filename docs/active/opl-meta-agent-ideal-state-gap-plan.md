...
| Script-to-pack evidence | 私有实现迁移台账、`contracts/script_to_pack_gate_receipt.json`、source-purity guards、active script caller scan、`source_ref_integrity_guard`、`retirement_readback_cleanup_guard`、`script_to_pack_retirement_gates` 和 policy contracts。 | 后续增长时继续证明脚本只是 materializer/helper 或 repo-native verification wrapper，并有真实 active caller 与 repo-local script ref integrity；新增或继续稳定下来的规则继续迁入 declarative pack、contracts 或 OPL primitive；未进入 gate、caller graph 或 source-ref integrity 的新增脚本必须 fail closed。Cleanup/readback 可以识别候选和缺失证据，但不能授权 physical delete、owner receipt、typed blocker instance、OPL primitive parity、App/registry/generated-hosted readiness、default promotion 或 target/domain/production readiness。脚本物理退役仍需要 OPL primitive parity、no-active-caller 和 tombstone/provenance refs。 |
