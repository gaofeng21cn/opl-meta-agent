...
- Script-to-pack 细节的 SSOT 是 [私有实现与 OPL 迁移台账](./opl-private-implementation-migration-inventory.md)、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt` / `#script_morphology_policy`、`contracts/script_to_pack_gate_receipt.json`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、`npm run script-to-pack:readback` / `npm run script-to-pack:readback:full` 和 source-purity tests。本文只保留当前读法：missing-machine-gate blocker 已关闭为 closed structure gate，但保留脚本仍必须有真实 active caller、repo-local source refs、false-authority/no-forbidden-write 边界、OPL primitive parity 或 explicit fixture/proof refs、no-active-caller 和 tombstone/provenance 证据；readback 只能列 cleanup candidate、missing evidence、owner-delta route 和 typed-blocker ref shape，不能授权 physical delete、owner receipt、typed blocker instance、OPL primitive parity、App/registry/generated readiness、default promotion、target/domain/production readiness 或 script retirement。
