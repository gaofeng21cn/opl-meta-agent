...
| Target-agent handoff vocabulary | Producer 已统一为完整 target identity、declarative suite seed、canonical Foundry evaluation work order 和 external result judgment；旧 action-level `new_agent_delivery_gate` readback 已删除。App drilldown、repeat cohort、real source patch-loop、reviewer direct evidence 和 target owner receipt 样本继续走后置 evidence lane。 | `scripts/lib/foundry-lab-work-order.ts`、action output schemas、`scripts/build-agent-baseline.ts`、`scripts/takeover-agent.ts`、`scripts/improve-from-agent-lab-suite.ts`、OPL evaluation readback、target owner receipts / typed blockers、`docs/status.md` |
