...
| Retired legacy alias and wrapper tails | `retired_and_guarded` | `takeover:test --fixture` no parser branch / ordinary unknown-argument fail-closed、`bootstrap:sample` absence、no-closeout fixture graph negative tests、source-purity guards | 退役 refs 只允许出现在 negative guard、tombstone/provenance 和 history/process；不得恢复 compatibility alias、fallback route、repo-local plugin/Skill/product wrapper、sidecar/status/workbench shell。 |
