...
- Script-to-pack 细节的 SSOT 是 `docs/active/opl-private-implementation-migration-inventory.md`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/active_caller_scan_policy`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/source_ref_integrity_guard`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`contracts/script_to_pack_gate_receipt.json`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、`npm run script-to-pack:readback`、`npm run script-to-pack:readback:full` 和 source-purity tests。当前 machine gate receipt 已关闭 `script_to_pack_hygiene` 的 missing-machine-gate blocker，并在 live-progress / owner-chain contracts 中记录为 closed structure gate；这只证明当前脚本形态、active caller、source-ref integrity、retention gate、OPL surface consumption、cleanup candidate readback 和 no-resurrection 边界可机读，不声明 OPL primitive parity、脚本物理退役、no-active-caller、tombstone/provenance、target-agent ready 或 production ready。保留脚本必须继续有真实 active caller graph 证据，且不能靠 source-purity self-guard refs 自身字符串自证；active caller scan 的 scan inputs、policy id、自证禁令和 fail-closed conditions 已由 machine policy + receipt 双向绑定，不再只靠测试 helper 约定。`source_ref_integrity_guard` 要求所有脚本 machine refs 保持为 `scripts/` 下 repo-local `.ts` / `.sh`，防止 stale / alias / 外部路径形成第二真相，但不授权 missing ref 修复、physical delete、primitive parity、readiness 或 default promotion。默认 `script-to-pack:readback` 从同一来源派生 compact operator readback；`script-to-pack:readback:full` 从 receipt、分类、active caller 和 retirement gates 派生每个 retained script 的 missing evidence、owner-delta route 与 typed-blocker ref shape，且所有 rows 均 `can_apply_cleanup=false`；这些 readback 不是删除授权、typed blocker instance、owner receipt 或 readiness evidence。`contracts/script_to_pack_gate_receipt.json#reciprocal_consumption_policy` 还约束 receipt 的 `consumed_by_refs` 与 live-progress、owner-chain、production-acceptance、functional-audit 消费方互相回指，防止 active read surface 漂成第二真相；该 guard 仍不是 retirement、primitive parity 或 readiness evidence。`tests/source-purity.test.ts#script-morphology-gate` 保留 receipt anchor；script morphology / active caller self-guard / source-ref integrity / reciprocal consumption guard 拆到 `tests/source-purity/*.test-case.ts` 和 `tests/source-purity/support.ts`，并由 `contracts/script_to_pack_gate_receipt.json#machine_gate_inputs.source_purity_test_structure` 声明为不能证明 active caller、retirement 或 readiness 的 self-guard refs；`tests/source-purity-boundary.test.ts` 持有 source-shape / second-framework 边界测试并纳入 smoke。
...
- Strict source-purity 仍有 `scripts/` materializer/helper 收薄和 OPL primitive replacement 尾项；per-script 分类、active caller graph、已迁回合同的 policy 细节和退役门回到私有实现迁移台账与 machine guards。
...
| Source shape / no-resurrection guard | `generic_shell_absent_with_script_hygiene_tail` | `contracts/functional_privatization_audit.json`、`contracts/default_caller_deletion_evidence.json`、`runtime/authority_functions/meta-agent-authority-functions.json`、source-purity tests | `functional_structure_gap_count=0` 只证明 repo-owned wrapper/runtime/workbench shell absent；scripts/materializers 仍按 script-to-pack gate 持续收薄。 |
...
| Stage-Native compiler refs | `done_as_ref_templates_with_contract_owned_vocabulary` | `contracts/stage_control_plane.json`、`contracts/stage_native_artifact_vocabulary.json`、`scripts/lib/stage-native-artifact-contract.ts`、`scripts/lib/stage-decomposition-pack-draft-parts/`、`tests/stage-decomposition-materializer.test.ts`、`tests/contracts-stage-control.test.ts` | 生成 target agent pack 时包含 physical kernel locator、conformance 和 workbench consumption refs-only templates；Stage folder files、stage/attempt/manifest/lineage required fields、physical kernel entries、conformance units、workbench projections、output roles 和 false-authority flags 已由 `contracts/stage_native_artifact_vocabulary.json` 持有，helper 只消费合同生成动态 refs。不创建 runtime state、不 owner promotion、不管理 target worktree、不写 owner receipt body。旧 `stage-decomposition-pack-draft.ts` barrel re-export 已退役；closeout fixture builder、pack validator、file materializer 和 shared refs/helper 由 concrete parts 直接承接并纳入 machine gate；这仍是 retained materializer/helper，不是 OPL primitive parity 或整组退役授权。 |
...
| Script morphology | `active_hygiene_with_machine_receipt` | `runtime/authority_functions/`、`contracts/script_to_pack_gate_receipt.json`、`scripts/lib/*`、私有实现迁移台账 | 当前脚本只允许是 authority implementation、smoke helper、fixture/proof helper 或 developer work-order materializer；当前 machine gate receipt 已证明扫描范围、caller graph 和 no-resurrection gate 可消费，但不授权 OPL primitive parity、脚本退役、第二 Agent Lab、promotion gate、registry/App shell、worktree lifecycle owner 或 target truth writer。 |
| Script-to-pack retirement gates | `machine_gate_receipt_closed_with_future_retirement_evidence_tail` | `contracts/script_to_pack_gate_receipt.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/active_caller_scan_policy`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/source_ref_integrity_guard`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/allowed_opl_surface_consumption_policy_ref`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、`contracts/developer_work_order_policy.json`、`contracts/standard_foundry_policies.json`、`contracts/stage_native_artifact_vocabulary.json`、source-purity tests、`tests/stage-decomposition-materializer.test.ts` | Repo-local TypeScript scripts and shell verification wrappers under `scripts/` have a current machine gate receipt and must keep explicit active-caller evidence, source-ref integrity and retention / retirement gates; the scan scope is machine-declared as `.ts` + `.sh`, with no exempt shell wrappers. Any `consumes_opl_surfaces` entry must be admitted by the private policy allowlist, match an allowed script class, and keep runtime truth / target truth / owner receipt body / promotion-style claims false. Materializer groups still require OPL primitive parity, target-agent generic handoff / explicit fixture path, no-forbidden-write, no-active-caller and tombstone/provenance refs before absorb or retirement; shell wrappers stay limited to repo-native verification, hygiene and external-cache-boundary roles until OPL verification/hygiene parity plus no-active-caller and tombstone/provenance refs exist. The active caller scan is recalculated by source-purity tests from `active_caller_scan_policy` and fails closed if a retained script is orphaned, has a missing caller ref, is absent from retirement gates, is only self-proven by source-purity self-guard refs, or carries stale / external / non-`scripts/` machine refs. The receipt's reciprocal consumption policy also fails closed if live-progress, owner-chain, production-acceptance or functional-audit consumers stop carrying the receipt ref / contract ref / accepted shape / false-authority guard. Developer work-order policy, target-improvement default change-ref policy, standard Foundry policy, Stage Native Artifact vocabulary, stage pack defaults, shared policy release, allowed OPL surface consumption refs and forbidden owner roles already have contract refs; retired standalone projection helpers must not return. |
...
| Standard Agent source shape purity | `generic_shell_absent_with_script_hygiene_tail` | `contracts/functional_privatization_audit.json`、`contracts/default_caller_deletion_evidence.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt`、source-purity tests、本文 | `functional_structure_gap_count=0` 只说明 repo-owned generic shell absent；default caller、sidecar、workbench 和 generated shell 只保留 OPL-owned refs 与 OMA boundary evidence tail。Scripts/materializers 仍是 strict source-purity hygiene tail，不能扩成 runner / promotion gate / workbench。 |
...
| Script-to-pack hygiene | 当前 missing-machine-gate blocker 已由 `contracts/script_to_pack_gate_receipt.json` 关闭为 closed structure gate；保留脚本仍必须证明只是 authority implementation、smoke/proof helper、thin delegation、developer work-order materializer 或 repo-native verification wrapper，并且必须有真实 npm/import/shell/test caller；新增或增长的 `.ts` / `.sh` repo-local script 都先回到 machine gate 分类。Active caller scan policy 和 source-ref integrity guard 已从测试 helper 上收到 authority-functions contract 并由 receipt 回指，减少测试逻辑和机器合同的第二真相风险。Receipt consumption policy 已把 live-progress、owner-chain、production-acceptance 和 functional-audit 的消费回指纳入 machine guard，减少 refs-only read surface 漂移风险。 | `contracts/script_to_pack_gate_receipt.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/active_caller_scan_policy`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/source_ref_integrity_guard`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、source-purity tests、私有实现迁移台账 |
...
| Script-to-pack evidence | 私有实现迁移台账、`contracts/script_to_pack_gate_receipt.json`、source-purity guards、active script caller scan、`source_ref_integrity_guard`、`retirement_readback_cleanup_guard`、`script_to_pack_retirement_gates`、policy contracts、`npm run source-structure:json` compact 机器 readback、`npm run script-to-pack:readback` compact operator readback，以及 `npm run script-to-pack:readback:full` 完整 cleanup candidate readback。 | 后续增长时继续证明脚本只是 materializer/helper 或 repo-native verification wrapper，并有真实 active caller 与 repo-local script ref integrity；新增或继续稳定下来的规则继续迁入 declarative pack、contracts 或 OPL primitive；未进入 gate、caller graph 或 source-ref integrity 的新增脚本必须 fail closed。JSON/readback 可机器读取 line-budget findings、script-to-pack guard 状态、violation list、compact cleanup summary、cleanup candidates、missing evidence、owner-delta route 和 typed-blocker ref shape，但不能授权 physical delete、owner receipt、typed blocker instance、OPL primitive parity、App/registry/generated-hosted readiness、default promotion 或 target/domain/production readiness。脚本物理退役仍需要 OPL primitive parity、no-active-caller 和 tombstone/provenance refs。 |
...
6. `script_to_pack_hygiene`
   当前 missing-machine-gate blocker 已关闭，source-ref integrity 和 cleanup/readback guard 已进入 script morphology policy 和 receipt；后续工作是持续减少仍需保留的 materializer/helper 语义，而不是扩大脚本职责。每次新增或收薄脚本，都先回到私有实现迁移台账、`contracts/script_to_pack_gate_receipt.json`、`active_script_caller_scan`、`source_ref_integrity_guard`、`retirement_readback_cleanup_guard` 和 `script_to_pack_retirement_gates` 分类，再用 source-purity tests 验证 no-resurrection / no-forbidden-surface / source-ref no-second-truth / cleanup-readback false-ready boundary；新增保留脚本如果没有真实 active caller、只被 source-purity 自身字符串引用，或 source refs 不是 repo-local `scripts/` 下 `.ts` / `.sh`，必须 fail closed。`scripts/lib/domain-pack.ts` 这类共享 pack helper 只能保留为 pack summary / deterministic target-agent fixture helper，不能升级为 OPL pack compiler、generated interface owner、scaffold generator owner、target pack authority 或 target truth writer。
...
- 推进 `registry_app_consumption`、`new_agent_consumption_scaleout`、`real_blocked_target_patch_loop_scaleout`、`independent_codex_reviewer_attempt`、`standard_target_agent_handoff_convergence`，并把 `script_to_pack_hygiene` 保持为 closed-structure-gate 后的持续退役/上收治理。
...
- 若推进 script-to-pack hygiene，先读取 `contracts/script_to_pack_gate_receipt.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/source_ref_integrity_guard`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/retirement_readback_cleanup_guard`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、`npm run script-to-pack:readback` 和 `npm run script-to-pack:readback:full`，按对应 gate 补齐真实 active caller、repo-local source refs、OPL primitive parity、target-agent generic handoff / explicit fixture path、no-forbidden-write、no-active-caller 和 tombstone/provenance refs；任何新增 `consumes_opl_surfaces` 必须先进入 allowlist 并保持 false-authority flags，不要用口头判断替代退役证据，也不要把当前 receipt、source-ref integrity guard 或 cleanup readback guard/readback output 写成 retirement、physical delete、readiness 或 parity claim。
