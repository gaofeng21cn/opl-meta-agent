...
- Script-to-pack 细节的 SSOT 是 `docs/active/opl-private-implementation-migration-inventory.md`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/active_caller_scan_policy`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs` 和 source-purity tests。本文只保留当前 gap 读法：脚本面仍是 strict source-purity hygiene tail；保留脚本必须有真实 active caller graph 证据，且不能靠 `tests/source-purity.test.ts` 自身字符串自证；active caller scan 的 scan inputs、policy id、自证禁令和 fail-closed conditions 已由 machine policy + receipt 双向绑定，不再只靠测试 helper 约定；没有对应 machine gate 证据时，不能写成已退役、已由 OPL 接管或可恢复 compatibility surface。`tests/source-purity.test.ts` 持有 script morphology / active caller self-guard；`tests/source-purity-boundary.test.ts` 持有 source-shape / second-framework 边界测试并纳入 smoke。
...
| Script-to-pack retirement gates | `active_machine_gate_with_policy_contracts` | `runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/active_caller_scan_policy`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/allowed_opl_surface_consumption_policy_ref`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、`contracts/developer_work_order_policy.json`、`contracts/standard_foundry_policies.json`、`contracts/stage_native_artifact_vocabulary.json`、source-purity tests、`tests/stage-decomposition-materializer.test.ts` | Repo-local TypeScript scripts and shell verification wrappers under `scripts/` must have explicit active-caller evidence and retention / retirement gates; the scan scope is machine-declared as `.ts` + `.sh`, with no exempt shell wrappers. Any `consumes_opl_surfaces` entry must be admitted by the private policy allowlist, match an allowed script class, and keep runtime truth / target truth / owner receipt body / promotion-style claims false. Materializer groups must use OPL primitive parity, target-agent generic handoff / explicit fixture path, no-forbidden-write, no-active-caller and tombstone/provenance refs before absorb or retirement; shell wrappers must stay limited to repo-native verification, hygiene and external-cache-boundary roles until OPL verification/hygiene parity plus no-active-caller and tombstone/provenance refs exist. The active caller scan is recalculated by source-purity tests from `active_caller_scan_policy` and fails closed if a retained script is orphaned, has a missing caller ref, is absent from retirement gates, or is only self-proven by the source-purity test. Developer work-order policy, target-improvement default change-ref policy, standard Foundry policy, Stage Native Artifact vocabulary, stage pack defaults, shared policy release, allowed OPL surface consumption refs and forbidden owner roles already have contract refs; retired standalone projection helpers must not return. |
...
| Script-to-pack evidence | 私有实现迁移台账、source-purity guards、active script caller scan、`script_to_pack_retirement_gates` 和 policy contracts。 | 后续增长时继续证明脚本只是 materializer/helper 或 repo-native verification wrapper，并有真实 active caller；新增或继续稳定下来的规则继续迁入 declarative pack、contracts 或 OPL primitive；未进入 gate 或 caller graph 的新增脚本必须 fail closed。 |
...
- 若推进 script-to-pack hygiene，先读取 `runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt.active_script_caller_scan`、`runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates` 和 `contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`，按对应 gate 补齐真实 active caller、OPL primitive parity、target-agent generic handoff / explicit fixture path、no-forbidden-write、no-active-caller 和 tombstone/provenance refs；任何新增 `consumes_opl_surfaces` 必须先进入 allowlist 并保持 false-authority flags，不要用口头判断替代退役证据。
