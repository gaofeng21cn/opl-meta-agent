...
| Foundry Agent series pin | `done_for_structural_conformance` | `contracts/foundry_agent_series.json`、`scripts/lib/stage-decomposition-pack-draft/builder.ts`、`tests/contracts-foundry-series.test.ts`、`tests/bootstrap-loop.test.ts`、OPL `agents conformance --agent oma=...` | repo 与生成 target 均显式声明跨域 truth/memory/artifact/quality authority 为 false；target action schema pair、required pack paths 与 build digests 一起由 materialization 回归覆盖。该结构通过不授权 domain ready、quality/export verdict、artifact/memory body、owner receipt 或 default promotion。 |
