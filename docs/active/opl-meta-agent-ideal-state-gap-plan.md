...
| Script-to-pack retirement gates | `active_machine_gate_with_policy_contracts` | `runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`contracts/developer_work_order_policy.json`、`contracts/standard_foundry_policies.json`、`tests/source-purity.test.ts`、`tests/stage-decomposition-materializer.test.ts` | TS materializers/helpers and repo shell verification wrappers must have explicit retention / retirement gates. Materializer groups must use OPL primitive parity, target-agent generic handoff / explicit fixture path, no-forbidden-write, no-active-caller and tombstone/provenance refs before absorb or retirement; shell wrappers must stay limited to repo-native verification, hygiene and external-cache-boundary roles until OPL verification/hygiene parity plus no-active-caller and tombstone/provenance refs exist. Developer work-order policy, target-improvement default change-ref policy, standard Foundry policy, stage pack defaults, shared policy release and forbidden owner roles already have contract refs; retired standalone projection helpers must not return. |
...
| Script-to-pack hygiene | 保留脚本必须继续证明只是 authority implementation、smoke/proof helper、thin delegation、developer work-order materializer 或 repo-native verification wrapper；新增或增长脚本先回到 machine gate 分类。 | `runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates`、`tests/source-purity.test.ts`、私有实现迁移台账 |
