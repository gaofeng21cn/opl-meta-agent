...
5. `script_to_pack_hygiene`
   把仍在 scripts 中增长的 agent-building 规则优先迁入 declarative pack 或 contracts；保留脚本必须能指向 `runtime/authority_functions`、smoke action、fixture/proof helper 或 developer work-order materializer，并持续证明不持有 OPL generic runtime、target domain truth 或 default promotion authority。当前已完成的低风险收薄是 `scripts/lib/work-order-policy.ts`，用于复用 closeout refs、runtime/read-model proof、workspace proof 和 forbidden target surface 默认面；后续仍需把稳定 AI reviewer gate、capability mapping 和 proposal policy 文本继续迁回 `agent/` 与 `contracts/`。
