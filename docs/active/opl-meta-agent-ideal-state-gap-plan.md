...
| App/workbench live consumption | App projection contract、drilldown readiness receipt、OPL production-consumption refs-only gate 已可消费 OMA evidence，当前 long-soak ref 已 verified，历史 typed blocker 仅为 provenance；`publication-brief-agent` 新消费样本已验证 generated product-entry / workbench descriptors ready；OMA baseline-owner review replay workorder 已有 domain-owned typed blocker verified receipt | 后续 target cohort 的真实 App render/screenshot、live runtime drilldown closeout receipt 和 repeat long-soak evidence；当前 OPL-side consumption ready、新 Agent descriptor ready 和 replay typed blocker 都不授权 target default promotion、human approval 或 production closeout。 |
