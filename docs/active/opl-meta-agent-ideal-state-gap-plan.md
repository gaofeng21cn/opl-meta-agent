...
- 2026-07-07 blocked cleanup parity re-audit 结论：原 cleanup items `5/6/7` 本轮均不能安全物理删除。`5 line-budget aliases` 仍有 package/verify/test/machine readback caller，只能保留为 canonical `source-structure` 的 compatibility alias；`6 execute-external-work-order.ts` 仍是 OMA developer-work-order validation aperture，OPL 只证明 executor primitive parity；`7 stage-decomposition-pack-draft-parts/**` 与 `work-order-*` helpers 仍缺 OPL Pack/Stage/WorkOrder full schema/materializer parity。该结论不声明 blocked complete，只把 blocker 改写为 owner route、required primitive、verification 和 tombstone 条件，细节归私有实现迁移台账。
