...
| Production consumption | `partial_blocked_by_domain_typed_blocker` | `contracts/production_acceptance/oma-production-consumption-long-soak-typed-blocker.json`、OPL App drilldown OMA production-consumption followthrough | OMA 已 materialize domain-owned typed blocker，声明 `long_soak_refs` 缺口不能被 contract 替代；OPL refs-only record/verify 尚待消费该 blocker，production closeout 仍未闭合。 |
...
自进化任务的边界按三方固定：Agent Lab 负责 suite run、AHE evidence/falsification read model、variant comparison、risk review、promotion gate 和 App/workbench refs；OMA 负责把这些 refs 与目标 handoff 转成 developer work order / target capability candidate / mechanism patch proposal / typed blocker；目标 domain agent 负责 owner receipt、domain truth、quality verdict、artifact authority 和 memory body。当前 OMA 已完成 work order 机器面，剩余完善不再是新增 OMA 私有 runner，而是真实 target patch loop 与更多标准 handoff 样本。
...
2026-05-19 的 physical source morphology 调研把本仓源码目标进一步固定：`agent/` 承载 agent-building semantic pack，`contracts/` 承载 machine handoff / registration / App projection / scaleout evidence，`runtime/authority_functions/` 承载最小 authority refs，`scripts/` 只能是这些 authority refs、smoke helpers 或 fixture/proof implementation。`scripts/bootstrap-sample-agent.ts`、`scripts/lib/meta-agent-loop.ts`、testing takeover 和 external-suite improvement 入口不能被写成 generic runtime、promotion engine、OPL registry、App workbench 或 target-domain owner；真实运行、Agent Lab execution、promotion gate、generated interface 和 workbench 继续归 OPL Framework。2026-05-21 的私有实现台账进一步把大型 script surface 分类到 `docs/active/opl-private-implementation-migration-inventory.md`，并把 `agent:evidence` 与 `improve:external-suite` 共享的 target patch-loop machine refs、runtime/read-model consumption proof、workspace environment proof、no-patch/source-patch closeout evidence 和默认 forbidden target surfaces 先抽成 work-order policy helper；2026-05-22 该 helper 又被拆成 `scripts/lib/work-order-policy-constants.ts`、`work-order-refs.ts`、`work-order-efficiency.ts`、`work-order-builders.ts` 与 `work-order-validation.ts`，避免 work-order readiness、refs intake、efficiency projection 和 shape validation 重新堆成单文件私有控制面。后续又把 `improve:external-suite` 的 target improvement policy、change-ref mapping、patch traceability matrix、editable surface inference 和 target file-hint lookup 抽成 `scripts/lib/target-improvement-policy.ts`。这些 helper 只是 developer work-order / target improvement refs materializer，不是 Agent Lab runner、promotion gate、queue、attempt ledger、registry、App shell 或 target truth owner。family-retire-legacy-oma 清理已移除 repo-local Codex plugin/Skill wrapper 和空 runtime adapter 占位目录，当前 runtime 源码形态只剩 `runtime/authority_functions/`，generated Skill/product-entry、sidecar/projection dispatch、status read model 和 workbench shell 均回到 OPL-owned generated/hosted surfaces。
...
2026-05-23 的 `skillnerds/xskill` clean-room 学习沿用同一边界：只吸收 skill evolution 组织模式，不引入对方实现。OMA 可学习 trajectory atomization、candidate buffer、per-skill version / canary evidence、team redaction / sync refs，并把它们压到 agent-building policy、reviewer evidence、developer work order、mechanism proposal 和 typed blocker 口径；xskill daemon/runtime、generic scheduler、skill installer、team server、storage model 和 execution loop 不进入本仓。promotion gate、Agent Lab、queue、attempt ledger、worktree lifecycle、absorb / cleanup 和 generated interfaces 仍归 OPL Framework。
...
- App/workbench projection 的本仓 contract 与 drilldown readiness receipt surface 已落地；OPL App/operator drilldown 已消费 OMA patch-loop refs，且显示 `opl_meta_agent_patch_loop_closed_count=2`、`opl_meta_agent_scaleout_target_count=2`。OMA 已提供 production-consumption typed blocker artifact，说明 open gate `long_soak_refs` 只能由真实 long-soak refs 或 OPL refs-only blocker record/verify 表达；仍缺 production long-soak、真实 App 截图/发布包或 live runtime drilldown closeout receipt。
...
3. `registry_app_consumption`
   OPL 主仓或 App 侧消费 `contracts/opl_domain_manifest_registration.json` 与 `contracts/app_workbench_projection.json`，留下真实 registry discovery receipt、App render / screenshot / runtime drilldown receipt。当前 OPL generated interface read-model 已能消费本仓 contracts，App/operator drilldown 已消费 OMA patch-loop refs；下一步必须 record/verify OMA-owned typed blocker 或补齐真实 `long_soak_refs`，消费结果只能展示 refs/status/receipt/blocker，不能写 target truth 或默认 promotion。
...
5. `script_to_pack_hygiene`
   把仍在 scripts 中增长的 agent-building 规则优先迁入 declarative pack 或 contracts；保留脚本必须能指向 `runtime/authority_functions`、smoke action、fixture/proof helper 或 developer work-order materializer，并持续证明不持有 OPL generic runtime、target domain truth 或 default promotion authority。当前已完成的低风险收薄是 work-order helper 自然拆分与 `scripts/lib/target-improvement-policy.ts`，用于复用 closeout refs、runtime/read-model proof、workspace proof、forbidden target surface 默认面、target improvement mapping、patch traceability 和 editable surface inference；xskill-derived trajectory atomization、candidate buffer、per-skill canary/version evidence、team redaction/sync 只能作为 pack/contract policy 或 refs-only work-order/proposal/blocker 词汇收敛，不能落成 repo-local scheduler、installer、daemon 或 team server。
