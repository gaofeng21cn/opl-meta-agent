...
- 当前 cleanup 结论：`execute-external-work-order.ts` 只保留 OMA developer-work-order schema/currentness validation aperture；`stage-decomposition-pack-draft/` 只保留 OMA typed judgment、domain request builder 与 OPL receipt validator。目标仓 IO、path/symlink safety、scaffold merge、pack compiler fold-in、materialized digest、final build receipt、worktree execute/absorb/cleanup 均不再由 OMA 默认路径承担。进一步物理删除仍需 generated action schema parity、no-active-caller、source-purity receipt 和 tombstone/provenance，不由 focused tests 单独授权。
