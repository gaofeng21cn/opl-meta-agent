...
- Script-to-pack 细节的 SSOT 是 [私有实现与 OPL 迁移台账](./opl-private-implementation-migration-inventory.md)、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt` / `#script_morphology_policy`、`contracts/script_to_pack_gate_receipt.json`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、`npm run script-to-pack:readback` / `npm run script-to-pack:readback:full` 和 source-purity tests。`agent/professional_skills/oma-work-order-hygiene/SKILL.md` 承接该面的 AI-first reviewer 判断，旧 `oma-script-to-pack-hygiene-reviewer` 只保留 redirect；输出 retain / absorb / delete / tombstone / route-back / work-order recommendation。本文只保留当前读法：missing-machine-gate blocker 已关闭为 closed structure gate，但保留脚本仍必须有真实 active caller、repo-local source refs、false-authority/no-forbidden-write 边界、OPL primitive parity 或 explicit fixture/proof refs、no-active-caller 和 tombstone/provenance 证据；readback 只能列 cleanup candidate、missing evidence、owner-delta route 和 typed-blocker ref shape，不能授权 physical delete、owner receipt、typed blocker instance、OPL primitive parity、App/registry/generated readiness、default promotion、target/domain/production readiness 或 script retirement。
...
- 2026-07-07 cleanup landing 结论：测试 support 不再 re-export Node 标准库，`external-suite-fixtures` 不再持有重复 repo/JSON helper；legacy professional skill 物理 `TOMBSTONE.md` 文件已合并为 `capability_map` / README redirect ledger；原 cleanup item `5 line-budget aliases` 已从 package scripts、`scripts/verify.sh` 和机器 policy/readback caller 面退役，canonical operator surface 只保留 `source-structure` / `source-structure:strict`。2026-07-07 parity re-audit 后，`6 execute-external-work-order.ts` 仍是 OMA developer-work-order validation aperture，OPL 已有 executor primitive 和部分 OMA guard，但本仓 action surface 仍有 active caller；`7 stage-decomposition-pack-draft/` 已改为 target pack draft 自然边界目录，和 `work-order-*` helpers 仍缺 OPL Pack/Stage/WorkOrder full schema/materializer parity。该结论不声明 broader script cleanup complete，细节归私有实现迁移台账。
...
- OMA 不维护 repo-owned generic plugin wrapper；目标 agent 必须生成 OPL Agent Package manifest sidecar，并通过 `opl connect agent-packages validate-manifest --manifest-url <sidecar> --json` 调用 OPL Connect 校验。OPL contracts / Connect validator 是 Agent Package manifest 机器 SSOT，Codex plugin 只是 OPL generated carrier projection；OMA 不复制第二套 package manifest 标准。
- OMA 维护 repo-owned rich primary skill：`agent/primary_skill/SKILL.md` 是 OMA 默认 Codex entry，覆盖用途、普通入口、Agent Lab / target-agent handoff、new-agent delivery gate、authority boundary、professional skill routing 和 false-completion guard。OPL 仍持有 generated carrier / manifest standard / validation；OMA 只消费标准并提供 domain-owned primary skill source。
...
| Stage-Native compiler refs | `done_as_ref_templates_with_contract_owned_vocabulary` | `contracts/stage_control_plane.json`、`contracts/stage_native_artifact_vocabulary.json`、`scripts/lib/stage-native-artifact-contract.ts`、`scripts/lib/stage-decomposition-pack-draft/`、`tests/stage-decomposition-materializer.test.ts`、`tests/contracts-stage-control.test.ts` | 生成 target agent pack 时包含 physical kernel locator、conformance 和 workbench consumption refs-only templates；Stage folder files、stage/attempt/manifest/lineage required fields、physical kernel entries、conformance units、workbench projections、output roles 和 false-authority flags 已由 `contracts/stage_native_artifact_vocabulary.json` 持有，helper 只消费合同生成动态 refs。不创建 runtime state、不 owner promotion、不管理 target worktree、不写 owner receipt body。旧 `stage-decomposition-pack-draft.ts` barrel re-export 已退役；closeout fixture builder、pack validator、file materializer、artifact morphology validator、stage-native contract validator 和 shared contract consumer 由自然边界目录直接承接并纳入 machine gate；这仍是 retained materializer/helper，不是 OPL primitive parity 或整组退役授权。 |
...
| Script-to-pack retirement gates | `machine_gate_receipt_closed_with_future_retirement_evidence_tail` | `contracts/script_to_pack_gate_receipt.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt` / `#script_morphology_policy`、`contracts/private_functional_surface_policy.json#allowed_opl_surface_consumption_refs`、source-purity tests、`npm run script-to-pack:readback` / `npm run script-to-pack:readback:full` | Repo-local `.ts` / `.sh` scripts have a current machine gate receipt, but retained scripts remain materializer/helper/verification-wrapper rows until private inventory and machine gates show active caller, repo-local source refs, allowed OPL-surface consumption, OPL primitive parity or explicit fixture/proof refs, no-forbidden-write, no-active-caller and tombstone/provenance. Readbacks and receipt reciprocity are guard/readback surfaces only; they cannot authorize delete, retirement, readiness, default promotion or owner receipt writes. |
...
| Script-to-pack hygiene | 当前 missing-machine-gate blocker 已由 machine receipt 关闭为 closed structure gate；保留或新增 `.ts` / `.sh` scripts 仍必须证明真实 caller、repo-local refs、允许的脚本 class、false-authority / no-forbidden-write 边界、退役门和 tombstone/provenance route。字段级 scan policy、source-ref integrity、reciprocal consumption 和 cleanup readback 规则回到 machine gates。 | `contracts/script_to_pack_gate_receipt.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt` / `#script_morphology_policy`、source-purity tests、私有实现迁移台账 |
| Target-agent handoff vocabulary | Default-path gate 已把 target-agent generic vocabulary、developer work-order shape、source morphology、owner route、generated surface consumption、private residue decision、accepted owner-answer shape 和 no-forbidden-write proof 纳入 fail-closed readback；takeover 默认入口也走同一 gate。App drilldown、repeat cohort、real source patch-loop、reviewer direct evidence 和 target owner receipt 样本走后置 evidence lane。 | `contracts/action_catalog.json#actions/build-agent-baseline/new_agent_delivery_gate`、`contracts/action_catalog.json#actions/takeover-target-agent-test/new_agent_delivery_gate`、`scripts/build-agent-baseline.ts#assertNewAgentDeliveryGate`、`scripts/takeover-agent.ts`、production acceptance contracts、OPL read models、target owner receipts / typed blockers、`docs/status.md` |
...
1. `script_to_pack_hygiene`
   ...
   2026-07-07 的 item `5/6/7` cleanup re-audit 后，本轮已关闭 item `5` alias surface：`line-budget` / `line-budget:strict` 不再是 package、verify 或 machine readback caller。剩余开放条件集中在 `execute-external-work-order.ts` 的 OPL generated action surface parity/no-active-caller，以及 stage-decomposition pack-draft natural-boundary directory 与 work-order helpers 的 OPL Pack/Stage/WorkOrder primitive 覆盖 typed packet validation、pack draft materialization、artifact morphology、Stage-Native refs template 和 work-order progress/no-forbidden-write validation。以上剩余条件未满足时，只能保留并继续收薄，不能用 OPL generic primitive 存在来删除 OMA-specific validation/materializer surface。
...
- 若推进 script-to-pack hygiene，先读取 script-to-pack receipt、script morphology policy、OPL surface allowlist、compact/full readback 和 source-purity tests，再按对应 gate 补齐真实 active caller、repo-local source refs、OPL primitive parity、target-agent generic handoff / explicit fixture path、no-forbidden-write、no-active-caller 和 tombstone/provenance refs；任何新增 OPL surface consumption 必须先进入 allowlist 并保持 false-authority flags，不要用口头判断替代退役证据，也不要把 receipt、source-ref guard 或 cleanup readback 写成 retirement、physical delete、readiness 或 parity claim。
