...
Producer 随后生成 canonical `opl_work_order_materialization_request.v2`：其中只携带 OMA-owned evaluation semantics、完整 target identity 和 source/reviewer/candidate refs。OPL Foundry Lab 才负责分配 work-order identity、物化并执行 canonical evaluation work order。OPL 是唯一 suite-plan compiler；OMA 不管理 lease/worktree/lifecycle，不本地执行 suite，也不生成 suite plan、suite result、Foundry execution receipt、target owner receipt、learning/scaleout ledger、promotion result 或 target-domain blocker body；只有 OPL 返回的 target-bound result / receipt 才能进入 `improve:external-suite --suite-result` 与 downstream owner closeout。参考设计来源、PDF/论文 pattern packet 和调研综合 refs 只作为架构、工作流、grounding、评估和 failure taxonomy inspiration，不是 target truth、runtime dependency、owner receipt 或 quality verdict。
