...
当前最小 producer 闭环由 `scripts/build-agent-baseline.ts` 的 `build-agent-baseline` action 承载：Codex Skill 先把用户自然语言归一为 target identity 和 brief，再消费 OPL profile selector/readback，或沿 `source_derived_design` / `research_driven_design` 形成 `ReferenceDesignPacket|ResearchSynthesisPacket -> TransferMap -> AgentPackPlan -> DesignAdmissionReceipt -> StageDecompositionSubpacketSet`。脚本只消费 action ordered route 上的 OPL StageRun query readbacks；route 未闭合时返回 typed continuation，闭合后校验 `stage-decomposition` 声明的 stage graph / file plan 与 `agent-skeleton-build` 声明的 file bodies，生成 domain-authored scaffold materialization request。OPL scaffold 负责目标仓路径安全、文件 IO、pack compiler fold-in、真实 digest 和最终 `AgentBuildReceipt`；OMA 不启动私有 stage attempt、不签 stage receipt，也不自签落盘 build receipt。
