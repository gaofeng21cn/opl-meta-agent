...
当前最小 producer 闭环由 `scripts/build-agent-baseline.ts` 的 `build-agent-baseline` action 承载：Codex Skill 先把用户自然语言归一为 target identity 和 brief，再消费 OPL profile selector/readback，或沿 `source_derived_design` / `research_driven_design` 形成 `ReferenceDesignPacket|ResearchSynthesisPacket -> TransferMap -> AgentPackPlan -> DesignAdmissionReceipt -> StageDecompositionSubpacketSet`。脚本启动 live Codex `stage-decomposition` attempt，或消费显式 typed closeout，只校验并物化 closeout 声明的 stage graph、pack files、profile/design refs、independent gate、quality gate 和 no-forbidden-write policy；物化后调用 OPL scaffold/interface projection，并保留 `AgentBuildReceipt` 作为构建证明。
