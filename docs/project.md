...
当前最小 producer 闭环由 `build-agent-baseline` 承载：Codex Skill 归一 target identity/brief并形成设计对象。OPL StageRun readbacks 只是 Codex input；任意 raw/partial 设计 artifact 可继续，Codex 可跳过、重复、逆向或 route-back。OPL scaffold 只负责路径安全、文件 IO、pack compiler fold-in、digest 和最终 `AgentBuildReceipt`，不裁决语义路线。
