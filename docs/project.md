...
`agent/` 是本仓 domain pack 的 repo-tracked 入口。当前必须保持 `knowledge/`、`prompts/`、`quality_gates/`、`skills/`、`stages/` 的可解析文件存在、非空且无占位；`agent/stages/manifest.json` 是 OPL Pack 的 domain-owned declarative stage source，并把 policy、prompt、knowledge、quality gate、action 与 transition refs 绑定到真实文件和 catalog。`contracts/stage_control_plane.json` 只保留为旧 consumer 的 generated compatibility aggregate。
...
当前最小 producer 闭环由 `scripts/build-agent-baseline.ts` 的 `build-agent-baseline` action 承载：Codex Skill 先把用户自然语言归一为 target identity 和 brief，再消费 OPL profile selector/readback，或沿 `source_derived_design` / `research_driven_design` 形成 `ReferenceDesignPacket|ResearchSynthesisPacket -> TransferMap -> AgentPackPlan -> DesignAdmissionReceipt -> StageDecompositionSubpacketSet`。脚本只消费 action ordered route 上的 OPL StageRun query readbacks；route 未闭合时返回 typed continuation，闭合后校验 `stage-decomposition` 声明的 stage graph / file plan 与 `agent-skeleton-build` 声明的 file bodies，再物化 profile/design refs、independent gate、quality gate 和 no-forbidden-write policy。随后调用 OPL scaffold/interface projection，并保留 `AgentBuildReceipt` 作为构建证明；OMA 不启动私有 stage attempt 或签 stage receipt。

Producer 随后生成 declarative Agent Lab suite seed 与 canonical Foundry evaluation work order。work order 绑定 `domain_id + target_agent_ref + descriptor_ref`、suite/task identity 和 canonical source/reviewer/candidate provenance，并交给 `opl agent-lab evaluation-work-order execute --work-order <work-order.json> --output <dir>`。OMA 不本地执行 suite，不生成 suite result、Foundry execution receipt、target owner receipt、learning/scaleout ledger、promotion result 或 target-domain blocker body；只有 OPL 返回的 target-bound result / receipt 才能进入 `improve:external-suite --suite-result` 与 downstream owner closeout。参考设计来源、PDF/论文 pattern packet 和调研综合 refs 只作为架构、工作流、grounding、评估和 failure taxonomy inspiration，不是 target truth、runtime dependency、owner receipt 或 quality verdict。
