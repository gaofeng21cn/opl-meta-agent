...
| Source-purity guard | repo-owned wrapper/runtime surface 仍保持缺席；retained scripts have machine-checked active callers | `contracts/functional_privatization_audit.json`、`runtime/authority_functions/meta-agent-authority-functions.json#source_purity_scan_receipt`、source-purity tests | `scripts/` 只能是 authority implementation、smoke/proof helper、fixture helper、thin delegation 或 developer work-order materializer；新增保留脚本不能靠 source-purity 自身 guard 字符串自证 active caller。 |
