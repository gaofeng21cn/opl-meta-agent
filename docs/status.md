...
| Stage size and prompt principle | 11 个 active prompt 已收敛为目标、好结果、专业依赖/authority boundary 和 closeout；artifact morphology/representative task 在 graph 前参与判断，owner split 从开始贯穿；一个 top-level Stage 只承载一个主要开放语义判断，source step 可 merge/stage-internal/reject；机械生成、校验、物化、receipt/readback 留在 Stage 内 | `agent/prompts/*.md`、`agent/professional_skills/oma-stage-pack-intent-architecture/SKILL.md`、`contracts/stage_operating_principles.json`、`tests/contracts-domain-pack.test.ts` | `StageDecompositionSubpacketSet` 守 provenance/boundary，不规定认知顺序；schema/validator 不做 stage-size selector；专业因果依赖保留，普通工具/迭代顺序由 executor 决定。该结构不构成 target delivery 或 runtime evidence。 |
