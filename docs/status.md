...
OPL 标准消费的 live StageRun progress 合同是 `contracts/live_stage_run_progress_evidence.json`。该文件使用 `surface_kind=domain_live_stage_run_progress_evidence`、`status=owner_typed_blocker_recorded_not_ready_claim` 与 `opl_consumption_status=not_ready_by_domain_owned_typed_blocker_refs`，把 OMA-owned target-agent progress refs 暴露为 `refs.owner_receipt_refs`、`refs.typed_blocker_refs`、`refs.human_gate_refs`、`refs.quality_or_export_receipt_refs`、`refs.no_regression_refs` 和 `refs.long_soak_refs`，并提供 `doc_refs` / `next_verification_command_refs` 给 OPL 消费；OMA 私有来源标记保留在 `oma_surface_kind`。当前 owner receipt、quality/export receipt 与 long-soak refs 为空；`oma_baseline_owner_review` human gate 和 `registry_app_consumption`、`real_blocked_target_patch_loop_scaleout`、`independent_codex_reviewer_attempt`、`standard_target_agent_handoff_convergence`、`script_to_pack_hygiene` 五个尾项都以 OMA-owned typed blocker refs 表达。五个 target-agent owner evidence tail 的聚合 closure 是 `contracts/target_agent_owner_chain_evidence.json#target_agent_owner_evidence_tail_closure`，终态是 `blocked_by_domain_owned_typed_blocker_refs`；OPL 必须把它消费为 not-ready claim。`contracts/target_agent_owner_chain_evidence.json` 现在只作为输入/摘要引用，不再持有标准消费 lane；这些 blockers 不授权 target-domain ready、domain ready、production ready、quality/export verdict、owner receipt body、App live rendering、human approval 或 default promotion。
...
| Live StageRun progress evidence | OPL 标准消费 `contracts/live_stage_run_progress_evidence.json`；OMA 在该文件暴露 owner receipt / typed blocker / human gate / quality-export / no-regression / long-soak refs，并把缺真实 owner evidence 的尾项收束为 domain-owned typed blockers | `contracts/live_stage_run_progress_evidence.json`、`contracts/target_agent_owner_chain_evidence.json`、`contracts/production_acceptance/meta-agent-production-acceptance.json`、`tests/live-stage-run-progress-evidence.test.ts`、`tests/target-agent-owner-chain-evidence.test.ts` | 当前 owner receipt、quality/export receipt 和 long-soak refs 为空；human gate 与五个 target-agent owner evidence tail 以 typed blocker refs 表达；`opl_consumption_status` 必须保持 not-ready；不真实修改 target repo，不生成 actual candidate body，不持有 target domain ready、domain ready、production ready、target artifact authority、quality/export verdict、human approval、App live rendering 或 target owner receipt body。 |
| Target-agent owner-chain evidence | OMA 声明 OPL-hosted generated action path 的 live-progress ref shape readout，并把 `oma_baseline_owner_review` human gate missing receipt 与五个 target-agent owner evidence tail 收束为 domain-owned typed blocker closures；标准消费 lane 通过 `live_stage_run_progress_evidence_summary` 指向 `contracts/live_stage_run_progress_evidence.json` | `contracts/target_agent_owner_chain_evidence.json`、`contracts/stage_run_kernel_profile.json`、`contracts/action_catalog.json`、`contracts/live_stage_run_progress_evidence.json`、`tests/target-agent-owner-chain-evidence.test.ts` | 只接受 owner receipt / typed blocker / work-order receipt / no-regression refs；human gate closure 的 success receipt count 必须保持 0；五个 tail closure 的 owner receipt refs 必须保持空且 OPL 消费为 not-ready；不真实修改 target repo，不生成实际 candidate agent body，不持有 target domain ready、production ready、target artifact authority、quality/export verdict、human approval 或 target owner receipt body。 |
...
- `registry_app_consumption`：本仓已提供 refs-only discovery / App projection surface；真实 registry discovery receipt、App render/screenshot、live runtime drilldown closeout 和 repeat long-soak 证据仍归 OPL/App 侧落证。当前 typed blocker ref 见 `contracts/live_stage_run_progress_evidence.json#refs.typed_blocker_refs`。
- `real_blocked_target_patch_loop_scaleout`：当前多目标 refs-only scaleout gate 已由 `contracts/real_target_agent_scaleout_evidence.json` 记录 MAS/MAG 两个真实目标的 delivery receipt、owner receipt / typed blocker refs、Agent Lab result refs、no-forbidden-write proof 和 cleanup closeout refs；App/workbench projection 与 tests 固定 patch-loop machine refs 加 independent reviewer evidence/provenance refs。后续尾项是更多目标 cohort、真实 source patch / rerun、真实独立 reviewer invocation/context/trace/receipt 样本、App/OPL live consumption 和目标 owner gate 证据；这些 refs 不授权 domain ready、quality/export verdict、owner receipt body、App live rendering 或 default promotion。当前 typed blocker ref 见 `contracts/live_stage_run_progress_evidence.json#refs.typed_blocker_refs`。
- `independent_codex_reviewer_attempt`：结构化 reviewer schema、fail-closed rules 与 controlled canary independent gate refs 已存在；真实独立 invocation/context/trace/receipt、direct evidence、source refs、critique/suggestions/verdict/provenance 与 rollback/canary/version refs 仍是证据尾项。当前 typed blocker ref 见 `contracts/live_stage_run_progress_evidence.json#refs.typed_blocker_refs`。
- `standard_target_agent_handoff_convergence`：OMA 消费 target-agent generic handoff；MAS/MAG/RCA 和新 Foundry Agent 需要持续提供同一 vocabulary，不能在 OMA 或 Agent Lab 内新增 domain-specific command family。当前 typed blocker ref 见 `contracts/live_stage_run_progress_evidence.json#refs.typed_blocker_refs`。
- `script_to_pack_hygiene`：developer work-order 默认 policy 与 standard Foundry policy bundle 已迁入 contract-backed projection；后续新增或继续稳定下来的 agent-building policy 必须继续迁回 declarative pack、contracts、authority refs 或 OPL primitive；脚本退役只能按 machine gate 证明。当前 typed blocker ref 见 `contracts/live_stage_run_progress_evidence.json#refs.typed_blocker_refs`。
