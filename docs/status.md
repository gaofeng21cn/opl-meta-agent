...
| Professional skills | `agent/professional_skills/*/SKILL.md` 提供 repo-local Codex-style 专业方法技能；`contracts/capability_map.json` 覆盖全部 9 个 repo-local professional skills，并为 target improvement routing 登记 canonical paths、tokens、verification refs 和 forbidden surfaces | `contracts/pack_compiler_input.json`、`contracts/capability_map.json`、`agent/professional_skills/README.md`、stage prompt route notes、`git diff --check`、`scripts/verify.sh` | 只承载专家方法与路由说明；contracts 只登记 pack path / capability locator，不创建 runtime、target artifacts、owner receipts、typed blockers 或 promotion gate state。 |
