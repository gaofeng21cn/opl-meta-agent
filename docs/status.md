...
| Standard target repo generation | OMA 生成 target agent 时复用 OPL scaffold / validation / generated interface owner split；OMA 只填充领域语义、stage-decomposition refs、candidate package refs 和 owner-gated closeout refs | `scripts/build-agent-baseline.ts#runBuildAgentBaseline`、`docs/architecture.md#standard-target-agent-repo-generation`、`docs/decisions.md#target-agent-repo-目录标准归-opl-scaffoldoma-只写领域语义` | 目标目录标准归 OPL scaffold；README 只做人读索引；scaffold validation、generated interface readiness 或 suite pass 不能声明新智能体完成，必须进入 Agent Lab / reviewer / delivery gate / owner receipt 或 typed blocker。 |
...
| Domain skill declarations | `agent/skills/*.md` 提供 OPL/generated skill surface 可引用的 OMA domain skill declarations | `contracts/pack_compiler_input.json`、`contracts/stage_control_plane.json`、`tests/contracts-domain-pack.test.ts`、`tests/contracts-stage-control.test.ts` | README 只做人读索引，不作为 `required_domain_pack_paths`；不要在这里放 Codex-style 专业方法技能。 |
...
| Foundry Agent series | OMA 使用 canonical Foundry-series lifecycle，并声明 OMA-specific agent-building profile | `contracts/foundry_agent_series.json`、package dependency pin tests | OMA 不维护 repo-owned generic runtime wrapper；目标 agent 生成 OPL Agent Package manifest sidecar；标准 OMA Codex plugin 只是 repo-local materialized carrier，不是 package truth 或 domain authority。 |
...
| Work-order materialization | `build-agent-baseline`、`takeover:test`、`improve:external-suite`、`agent:evidence` 和 `execute:external-work-order` 已形成 refs-only action surface；`improve:external-suite` 接受通用 `target_agent_feedback_external_suite` 并把 MAS `high-quality-medical-manuscript` / `reviewer_revision` 作为特化 profile 投影为 developer work-order intake/readback；MAS `reviewer_revision` suite materialized 且 `contract_itself_triggers_execution=false` 时只显示 `runnable_pending`；`takeover:test` 默认入口现在也必须消费 AI reviewer evidence 并输出 delivery-gate readback | `contracts/action_catalog.json`、`contracts/developer_work_order_policy.json`、scripts、work-order tests | OMA 是 generic consumer，不拥有 runner、queue、target truth 或 target worktree lifecycle。可输出 receipt、candidate、work order、mechanism proposal、no-patch coordination receipt 或 typed blocker；需要的执行链是 `improve-from-external-agent-lab-suite -> execute-external-work-order -> target_owner_closeout_readback`；必须保留 agent-evolution decision ref、failure class、target owner route、editable/forbidden surfaces、expected behavior delta、verification refs、owner closeout readback、source Agent Lab result ref、reviewer evidence refs、OPL delegation aperture、target owner closeout refs 和 progress accounting；不能写 target truth、memory body、artifact body、quality/export verdict、owner receipt body 或 default promotion。 |
