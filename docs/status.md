...
| Source-derived design profile consumption | OMA 可在无内置 OPL profile 匹配但有论文/PDF/repo/产品案例 reference design 时生成 target agent pack | `scripts/lib/domain-pack.ts`、`scripts/build-agent-baseline.ts`、`scripts/lib/stage-decomposition-*`、`tests/bootstrap-loop.test.ts`、`tests/stage-decomposition-materializer.test.ts` | `profile_selection_mode=source_derived_design` 必须先生成并保留非空 `ReferenceDesignPacket -> TransferMap -> AgentPackPlan -> BuildReceipt`，再迁移到 target agent stage pack；四对象必须来自 pattern packet 或 pattern note，不能只是 raw source ref 或 identity shell；每个 source-derived stage requirement 必须有 `source_pattern_ref`/`stage_pattern_source_refs` 或 `target_only_requirement`。`BuildReceipt` 必须证明 source-derived stage refs、target-only requirements、rejected source patterns、forbidden claims 和 refs-only authority boundary。只消费 source refs / pattern packet refs / transferable pattern requirements / capability plan requirements，不复制外部 runtime、私有数据、target truth、domain verdict 或 promotion authority。 |
