...
| Professional skills | `agent/professional_skills/*/SKILL.md` 提供 repo-local Codex-style 专业方法技能；`oma-agent-evolution` 负责 OMA-owned diagnosis router 与 patch playbook | `contracts/pack_compiler_input.json`、`contracts/capability_map.json`、`agent/professional_skills/README.md`、stage prompt route notes、`git diff --check`、`scripts/verify.sh` | 只承载专家方法与路由说明；contracts 只登记 pack path / capability locator，不创建 runtime、target artifacts、owner receipts、typed blockers 或 promotion gate state。 |
