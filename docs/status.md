...
| Reference-design semantic resolver | OMA 已真实消费 OPL refs-only packet 与专家工作流 seed，而不是只保存 contract/test | `contracts/reference_design_packet.json`、`contracts/expert_workflow_pattern_library.json`、`scripts/lib/reference-design-workflow.ts`、`scripts/lib/domain-pack.ts`、`tests/bootstrap-loop.test.ts` | OPL handoff 只认 `opl.reference_design_pattern_packet.v1`；tracked fixture 的本地 JSON-pointer refs 解析为逐 step OMA `ReferenceDesignPacket`，每步生成并物化独立 stage。case-grounded 与 RCA seed 对同一 goal 产生不同 graph；raw/opaque source 和未覆盖的第二 source fail closed；user packet + seed 时 seed 只留 context，不进入 active graph。该状态只证明 repo behavior，不是 Live Evidence、target domain truth 或 runtime owner receipt。 |
