...
| Professional skill pack | `agent/professional_skills/*/SKILL.md` 提供 repo-local Codex-style agent-building 方法技能 | `contracts/pack_compiler_input.json`、`agent/skills/README.md`、stage prompt route notes、`git diff --check`、`scripts/verify.sh` | 只承载专业方法与路由说明；contracts 只登记 pack path，不创建 runtime、target artifacts、owner receipts、typed blockers 或 promotion gate state。 |
...
| Work-order materialization | `build-agent-baseline`、`takeover:test`、`improve:external-suite`、`agent:evidence` 和 `execute:external-work-order` 已形成 refs-only action surface；`improve:external-suite` 接受通用 `target_agent_feedback_external_suite` 并把 MAS `high-quality-medical-manuscript` / `reviewer_revision` 作为特化 profile 投影为 developer work-order intake/readback；`takeover:test` 默认入口现在也必须消费 AI reviewer evidence 并输出 delivery-gate readback | `contracts/action_catalog.json`、scripts、work-order tests | OMA 是 generic consumer，不拥有 runner、queue、target truth 或 target worktree lifecycle。可输出 receipt、candidate、work order、mechanism proposal、no-patch coordination receipt 或 typed blocker；必须保留 source Agent Lab result ref、feedback refs / source feedback refs、reviewer evidence refs、OPL delegation aperture、target owner closeout refs 和 progress accounting；不能写 target truth、memory body、artifact body、quality/export verdict、owner receipt body 或 default promotion。 |
