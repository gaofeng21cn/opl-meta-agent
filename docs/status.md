...
| Stage size design principle | Stage 大小现在按 AI-first / contract-light 读法落在 stage 主提示词和 `oma-stage-pack-intent-architecture` professional skill；一个 top-level stage 只承载一个主要开放语义判断，确定性生成、校验、物化、receipt 和 readback 留在 stage 内；保留大 stage 时用 typed subpacket / gate 暴露内部边界 | `agent/prompts/stage-decomposition.md`、`agent/professional_skills/oma-stage-pack-intent-architecture/SKILL.md`、`docs/decisions.md#stage-大小由-stage-prompt-和-professional-skill-的-ai-判断决定`、`docs/invariants.md`、`scripts/lib/domain-pack.ts#buildStageDecompositionSubpacketSet` | Schema、validator、tests 只守 refs、顺序、权限和 fail-closed 条件，不作为 stage-size selector；`StageDecompositionSubpacketSet` 不是 OPL runtime stage、私有子流程、target truth 或 owner verdict。 |
