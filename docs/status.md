...
- Developer work-order traceability：blocked suite 的 rubric gap 会被映射为 `patch_traceability_matrix`，包含 source failure refs、required patch refs、editable surfaces、目标仓文件提示、测试/receipt/禁止写入证明和版本 closeout 要求，避免 self-evolution 退化成泛泛 proposal 或前台开发者自由发挥。
