...
| Standard Foundry policy | OPL canonical contracts 持有 Stage-log、progress、typed-blocker、series design、stage pack、completion、projection 与 forbidden-role policy body；OMA 合同只保留 release pin、canonical refs 和 artifact-morphology/agent-building delta | `contracts/standard_foundry_policies.json`、`contracts/foundry_agent_series.json`、`scripts/lib/stage-decomposition-pack-draft/shared.ts`、source-purity tests、`tests/stage-decomposition-materializer.test.ts` | Stage-decomposition helper 通过 `opl-framework-shared/foundry-agent-series-policy` 读取 canonical policy 后合并 OMA delta；generated target series contract 同样只写 ref/pin/delta，不复制 canonical body。该结构不授权 target truth、quality/export、default promotion、stage content completion judgment 或 owner receipt body。 |
