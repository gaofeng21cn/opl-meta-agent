...
| Standard target repo generation | OMA 生成 target agent 时复用 OPL scaffold / validation / generated interface owner split；OMA 填充领域语义、stage-decomposition refs、candidate Agent Pack、AgentBuildReceipt、thin evaluation request 和 Foundry evaluation work order | `scripts/build-agent-baseline.ts#runBuildAgentBaseline`、`docs/architecture.md#standard-target-agent-repo-generation`、`docs/decisions.md#target-agent-repo-目录标准归-opl-scaffoldoma-只写领域语义` | 目标目录标准归 OPL scaffold；README 只做人读索引；AgentBuildReceipt、scaffold/interface validation、OPL-generated suite plan 或 work-order ready 不能声明新智能体完成，必须等待 OPL evaluation result、reviewer/self-evolution 和 target-owner closeout refs。 |
