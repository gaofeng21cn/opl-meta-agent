...
`contracts/domain_descriptor.json#/standard_agent_interface` 是 OMA 向 OPL 提供的标准接口真相源。它声明 workspace locator、runtime registration、progress aliases 与显式 routing signals；OMA 不实现 repo-owned direct CLI 或 runtime dispatch，因此相应 command template 明确为 `null`，由 OPL generated/hosted surface 与 Codex skill carrier承担入口。
...
| Standard Foundry policy / Framework hosting | OPL canonical contracts 持有 Stage-log、progress、typed-blocker、series design、stage pack、completion、projection 与 forbidden-role policy body；OPL Connect 持有 Framework link policy/materialization/repair；OMA 合同只保留 policy release pin、canonical refs 和 artifact-morphology/agent-building delta | `contracts/standard_foundry_policies.json`、`contracts/foundry_agent_series.json`、`contracts/opl_agent_package_manifest.json`、`scripts/run-with-repo-temp-env.sh`、`tests/contracts-foundry-series.test.ts`、`tests/managed-framework-link.test.ts` | Stage-decomposition helper 通过 OPL-managed link 解析 `opl-framework/foundry-agent-series-policy` 后合并 OMA delta；package/lock 不持有 Framework 或 Temporal 闭包，verification 只做只读 link check。generated target series contract 同样只写 ref/pin/delta，不复制 canonical body；该结构不授权 target truth、quality/export、default promotion、stage content completion judgment、runtime/release currentness 或 owner receipt body。 |
