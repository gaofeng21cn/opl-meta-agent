...
OPL family `Foundry Agent OS` 目标下，OMA 的 target delta 读 [OMA Foundry Agent OS 目标差异页](./active/foundry-agent-os-target-delta.md)：Agent Lab runtime/result/receipt ledger、evaluation/developer work-order execution、absorb/cleanup、scaffold generator、generated interface bundle、conformance/read-model projection、promotion/canary/rollback shell 和 capability registry ABI 上收到 OPL；agent-building semantics、candidate Agent Pack/AgentBuildReceipt、declarative suite seed、Foundry evaluation/developer work-order materialization、mechanism candidate、no-forbidden-write proof和 expected blocker / route-back refs 保留为 OMA authority kernel。该 target delta 只冻结架构方向，不声明 target-agent ready、App live rendering、human approval、default promotion 或 family production ready。
...
| Standard target repo generation | OMA 生成 target agent 时复用 OPL scaffold / validation / generated interface owner split；OMA 填充领域语义、stage-decomposition refs、candidate Agent Pack、AgentBuildReceipt、declarative suite seed 和 Foundry evaluation work order | `scripts/build-agent-baseline.ts#runBuildAgentBaseline`、`docs/architecture.md#standard-target-agent-repo-generation`、`docs/decisions.md#target-agent-repo-目录标准归-opl-scaffoldoma-只写领域语义` | 目标目录标准归 OPL scaffold；README 只做人读索引；AgentBuildReceipt、scaffold/interface validation 或 work-order ready 不能声明新智能体完成，必须等待 OPL evaluation result、reviewer/self-evolution 和 target-owner closeout refs。 |
...
| Declarative Stage Manifest | `agent/stages/manifest.json` 是 OPL Pack 的 domain-owned stage source，显式绑定 11 个 stage、action、transition、trust lane 与 pack refs | `agent/stages/manifest.json`、`contracts/pack_compiler_input.json`、`tests/contracts-domain-pack.test.ts`、generated-interface live readback | OPL 编译 hosted `family_stage_control_plane`；`contracts/stage_control_plane.json` 只保留旧 consumer compatibility aggregate，不可作为 fallback 或第二真相源。 |
| Professional skills | `agent/professional_skills/*/SKILL.md` 提供 repo-local Codex-style 专业方法技能；active capability 已收敛为 4 个 workflow-level skills，旧 9 个细粒度入口只保留 redirect/tombstone；当前方法层把 target-agent handoff vocabulary、baseline/delivery gate interpretation、trajectory learning triage 和 work-order hygiene 保留在这四个 skill 中，而不是新增更细 physical skill；`contracts/capability_map.json` 登记 active canonical paths、tokens、verification refs、forbidden surfaces 和 legacy redirect mapping | `contracts/pack_compiler_input.json`、`contracts/capability_map.json`、`agent/professional_skills/README.md`、stage prompt route notes、`git diff --check`、`scripts/verify.sh` | 只承载专家方法与路由说明；contracts 只登记 pack path / capability locator，不创建 runtime、target artifacts、owner receipts、typed blockers 或 promotion gate state。 |
...
| Work-order materialization | `build-agent-baseline`、`takeover:test` 和 `agent:evidence` 产出 target-bound declarative suite seed + canonical Foundry evaluation work order；`improve:external-suite` 强制消费显式 `--suite-result` 后才产出 capability judgment / developer work order；`execute:external-work-order` 只薄委托 OPL primitive | `contracts/action_catalog.json`、action schemas、`scripts/lib/foundry-lab-work-order.ts`、producer scripts、focused behavior tests | OMA 不拥有 Agent Lab runner/result/receipt ledger、target patch lifecycle、owner receipt 或 promotion gate。evaluation work order 固定完整 target identity、suite/task identity、canonical provenance、`consumer_dependency.status=available` 和 canonical OPL action；缺 external result/receipt 或 declarative work-order inputs 时只返回 pending/expected blocker ref。 |
...
和兼容面 `contracts/stage_control_plane.json`。前者的维护源是
...
和 `npm run stage-control:check` 维护；OPL Pack 的现役 stage compiler source 已切到
`agent/stages/manifest.json`，兼容 aggregate 不参与 fallback。两个 bundle manifest 都记录 source digest、
...
已用 Zoller 等 2026 年 case-grounded 血液病决策支持论文完成一条真实参考设计链路：`PDF -> MinerU Markdown -> opl.reference_design_pattern_packet.v1 -> OMA hybrid design -> 标准 target Agent Pack -> OPL Foundry evaluation -> OMA result consumption`。该次运行固定使用 OMA source candidate `3d329017879289d9074e30639f661ea740ed280c` 与 OPL Pack canonical `555c3189877f34f57b9a12f8dd78e8f0d471d32e`，没有追随未发布 Runway HEAD，也没有新增 OMA/OPL 私有 PDF parser runtime 或 `--reference-design-file` 旁路。

Source custody 与 parse-once 证据为：原始 PDF SHA256 `0cb414a420c3aaf11c02871c99fd068d6ab471b1ebdfc82f58995546235bc8a5`；MinerU Markdown `f5a7f94be22a65c485547b006210d6e030947ba85491284278c263828a883597`；pattern packet `b1c57ed218ef5867f64f608a5743f1bbca07608b454269f463162547c6e93bf6`；pattern notes `bb17dab45b828b2ff3a929ca47ed98316bcb128521dd9b6b4844d4440c2e813e`；extraction receipt `987304c666cdc14261f2e95dbf377079e2495f3a41738330ae00f40537878e0e`。论文只提供 structured case intake、attention-focused enrichment、hybrid routing、layered grounding、unsupported-evidence route、auditable aggregation、evaluation stack、model/runtime separation 和 validated specialized risk tools 等设计模式；血液病知识、患者数据、阈值、性能结论、runtime 与临床 authority 均未迁入目标 agent truth。

OMA 生成的目标 identity 是 `domain-agent:colorectal-surgery-risk`，`profile_selection_mode=hybrid`。本地 build-time readback 为 refs-only OPL profile conformance=`passed`、package schema/identity validation=`valid`、generated interface projection=`ready`；这些值不表示发布或安装完成，package proof 仍是 `candidate_sidecar_pending_publish`，`registry_url=null`、`live_download_proof=false`、`installed_reload_proof=false`，generated-surface handoff 仍只到 `descriptor_source_available`。最终 manifest 物化 10 个 stage：`structured-case-intake`、`attention-focused-enrichment`、`hybrid-mode-routing`、`layered-grounding`、`transparent-unsupported-evidence-route`、`auditable-context-aggregation`、`evaluation-stack`、`model-runtime-separation`、`validated-specialized-risk-tools` 与 target-only `agent-output-draft-owner-gate`；Framework-owned stage contract field residue=`0`。Stage manifest SHA256 为 `4f06ad413ab459a31f677063c68f48808e4a9ccfd0c4eb99fa8c88438ca52351`，AgentBuildReceipt SHA256 为 `b4f3897f1436bbe0f240bc9efcc63a223fbc16e232f1e28647b03aa15d456fc2`，build ref 为 `build-receipt-ref:opl-meta-agent/colorectal-surgery-risk`。

OPL Foundry work order `oma_foundry_lab_work_order_2899fbb99d2f` 的 SHA256 为 `e15848c70677b89a1bf63bd3719f486c30c163806d0c85e620fee7b3eee4b1fd`。无 observation packet 时，OPL 正确返回 `blocked_missing_evaluation_observations`、platform blocker `ofleb_21c0d68c7d9188031aea523b` 与 execution receipt `oflewr_0657d42841624ae171a860cc`，且 `suite_result_ref=null`。加入明确标注为 synthetic ABI-only 的 observations 后，OPL 返回 result `oals_6ee6f0098c1b02564e5d75f0`、execution receipt `oflewr_12427bc7f046368d52339c6b`、7 个 provenance bindings 和 `promotable_candidate_count=0`；下游原因仍为 `target_owner_closeout_required`。OMA 消费该结果后生成 candidate `oma_target_capability_candidate_670228cc499d`，traceability=`no_source_patch_required`，所有 target truth、memory、artifact、quality/export、promotion 与 model-weight authority flags 均保持 `false`。

该 E2E 证明 paper-to-agent 路径与 OPL Pack/Foundry ABI 可执行，并证明缺 observation 时 fail closed；它不证明肠癌手术风险内容的临床有效性、外部验证、domain owner acceptance、真实患者安全、默认 promotion 或 production readiness。当前合法停止条件是 target-domain owner 对真实领域资料、风险模型适用性、临床工作流、独立评审和真实 evaluation observations 完成 closeout，并返回 owner receipt 或 typed blocker。
