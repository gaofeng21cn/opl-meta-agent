...
OPL 标准消费的 live StageRun progress 合同是 `contracts/live_stage_run_progress_evidence.json`。该文件使用 `surface_kind=domain_live_stage_run_progress_evidence` 与 `status=owner_typed_blocker_recorded_not_ready_claim`，把 OMA-owned target-agent progress refs 暴露为 `refs.owner_receipt_refs`、`refs.typed_blocker_refs`、`refs.human_gate_refs`、`refs.quality_or_export_receipt_refs`、`refs.no_regression_refs` 和 `refs.long_soak_refs`，并提供 `doc_refs` / `next_verification_command_refs` 给 OPL 消费；OMA 私有来源标记保留在 `oma_surface_kind`。当前 owner receipt、quality/export receipt 与 long-soak refs 为空；`oma_baseline_owner_review` human gate 和 `registry_app_consumption`、`real_blocked_target_patch_loop_scaleout`、`independent_codex_reviewer_attempt`、`standard_target_agent_handoff_convergence`、`script_to_pack_hygiene` 五个尾项都以 OMA-owned typed blocker refs 表达。`contracts/target_agent_owner_chain_evidence.json` 现在只作为输入/摘要引用，不再持有标准消费 lane；这些 blockers 不授权 target-domain ready、domain ready、production ready、quality/export verdict、owner receipt body、App live rendering、human approval 或 default promotion。
...
| Live StageRun progress evidence | OPL 标准消费 `contracts/live_stage_run_progress_evidence.json`；OMA 在该文件暴露 owner receipt / typed blocker / human gate / quality-export / no-regression / long-soak refs，并把缺真实 owner evidence 的尾项收束为 domain-owned typed blockers | `contracts/live_stage_run_progress_evidence.json`、`contracts/target_agent_owner_chain_evidence.json`、`contracts/production_acceptance/meta-agent-production-acceptance.json`、`tests/live-stage-run-progress-evidence.test.ts`、`tests/target-agent-owner-chain-evidence.test.ts` | 当前 owner receipt、quality/export receipt 和 long-soak refs 为空；human gate 与五个 target-agent owner evidence tail 以 typed blocker refs 表达；不真实修改 target repo，不生成 actual candidate body，不持有 target domain ready、domain ready、production ready、target artifact authority、quality/export verdict、human approval、App live rendering 或 target owner receipt body。 |
| Target-agent owner-chain evidence | OMA 声明 OPL-hosted generated action path 的 live-progress ref shape readout，并把 `oma_baseline_owner_review` human gate missing receipt 收束为 domain-owned typed blocker closure；标准消费 lane 通过 `live_stage_run_progress_evidence_summary` 指向 `contracts/live_stage_run_progress_evidence.json` | `contracts/target_agent_owner_chain_evidence.json`、`contracts/stage_run_kernel_profile.json`、`contracts/action_catalog.json`、`contracts/live_stage_run_progress_evidence.json`、`tests/target-agent-owner-chain-evidence.test.ts` | 只接受 owner receipt / typed blocker / work-order receipt / no-regression refs；human gate closure 的 success receipt count 必须保持 0；不真实修改 target repo，不生成实际 candidate agent body，不持有 target domain ready、production ready、target artifact authority、quality/export verdict、human approval 或 target owner receipt body。 |
