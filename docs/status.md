...
| Agent-building professional skills | `agent/skills/oma-*.md` 与其他 `agent/skills/*.md` 共同构成 OMA domain skill pack；`oma-agent-evolution` 负责 OMA-owned diagnosis router 与 patch playbook | `contracts/pack_compiler_input.json`、`contracts/capability_map.json`、`agent/skills/README.md`、stage prompt route notes、`git diff --check`、`scripts/verify.sh` | 只承载方法与路由说明；contracts 只登记 pack path / capability locator，不创建 runtime、target artifacts、owner receipts、typed blockers 或 promotion gate state。 |
