...
| Source-derived design profile consumption | OMA 可在无内置 OPL profile 匹配但有论文/PDF/repo/产品案例 reference design 时生成 target agent pack | `scripts/lib/domain-pack.ts`、`scripts/build-agent-baseline.ts`、`scripts/lib/stage-decomposition-*`、`tests/bootstrap-loop.test.ts`、`tests/stage-decomposition-materializer.test.ts` | `profile_selection_mode=source_derived_design` 必须生成并保留非空 `ReferenceDesignPacket`、`TransferMap`、`AgentPackPlan` 三类设计对象；三对象必须来自 pattern packet 或 pattern note，不能只是 raw source ref 或 identity shell；进入物化前必须有 `DesignAdmissionReceipt` 证明 design-derived stage refs、target-only requirements、rejected source patterns、forbidden claims 和 refs-only authority boundary；`StageDecompositionSubpacketSet` 必须按 design-basis、transfer-planning、agent-pack-planning、design-admission、build-verification 保留 ordered chain；若 typed closeout 只缺可由有效设计对象推导的 subpacket 投影/refs，`build-agent-baseline` 会在 stage 内做一次有界 deterministic repair 后继续物化。物化后保留 `AgentBuildReceipt` / `build_receipt` 作为构建溯源与 conformance 证明。每个 source-derived stage 必须有 `stage_pattern_source_refs`。只消费 source refs / pattern packet refs / transferable pattern requirements / capability plan requirements，不复制外部 runtime、私有数据、target truth、domain verdict 或 promotion authority。 |
