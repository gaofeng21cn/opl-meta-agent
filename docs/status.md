...
| Work-order materialization | `build-agent-baseline`、`takeover:test`、`improve:external-suite`、`agent:evidence` 和 `execute:external-work-order` 已形成 refs-only action surface；`takeover:test` 默认入口现在也必须消费 AI reviewer evidence 并输出 delivery-gate readback | `contracts/action_catalog.json`、scripts、work-order tests | 可输出 receipt、candidate、work order、mechanism proposal、no-patch coordination receipt 或 typed blocker；不能写 target truth、memory body、artifact body、quality/export verdict、owner receipt body 或 default promotion。 |
