...
| Primary skill | `agent/primary_skill/SKILL.md` 是 OMA repo-owned rich default Codex entry；OMA-generated target agent 也必须生成 `agent/primary_skill/SKILL.md`，并在 target `contracts/capability_map.json` 与 package sidecar `codex_surface.primary_skill_ref` 中声明 | `agent/primary_skill/SKILL.md`、`contracts/capability_map.json`、`contracts/pack_compiler_input.json`、`scripts/build-agent-baseline.ts`、`tests/contracts-domain-pack.test.ts`、`tests/bootstrap-loop.test.ts` | OPL owns standard、manifest validation 和 generated carrier projection；carrier 是 OPL materializer 生成的完整 `SKILL.md` copy，不是 membership/status 轴，也不能声明 domain ready。OMA consumes the standard and emits repo-owned source/candidate refs。Professional skills 继续非默认全局暴露，只由 stage / primary skill 按需路由。 |
...
| Foundry Agent series | OMA 使用 canonical Foundry-series lifecycle，并声明 OMA-specific agent-building profile | `contracts/foundry_agent_series.json`、package dependency pin tests | OMA 不维护 repo-owned generic plugin wrapper；目标 agent 生成 OPL Agent Package manifest sidecar，Codex plugin 只是 OPL generated carrier projection。 |
