...
2026-06-09 script-to-pack active-plan SSOT thinning tranche:

- Theme / SSOT: script-to-pack hygiene, retired wrapper no-resurrection and strict source-purity current reading. The active gap plan remains the single Active Truth owner for completion/gap/next prompt, but the detailed script classification SSOT is `docs/active/opl-private-implementation-migration-inventory.md` plus `runtime/authority_functions/meta-agent-authority-functions.json#script_morphology_policy/script_to_pack_retirement_gates` and `tests/source-purity.test.ts`.
- Peer docs: `docs/active/opl-meta-agent-ideal-state-gap-plan.md` = active-plan owner and thinned; `docs/active/opl-private-implementation-migration-inventory.md` = `more_specific_detail` and retained as SSOT for per-script active caller / class / authority / migration gate; `docs/status.md` and `docs/invariants.md` = support current truth / forbidden-claim boundary; `docs/docs_portfolio_consolidation.md` = lifecycle role map; this process index = `history_or_provenance`.
- Section classification: active-plan current conclusion script-to-pack bullet = `covered_by_ssot` detail thinned to pointer plus current gap readout; active-plan strict source-purity long bullet list = mixed `covered_by_ssot` / `more_specific_detail` and replaced by three open-tail rows; active-plan next-round `script_to_pack_hygiene` scope = `covered_by_ssot` detail thinned to gate-first action; no conflicts with SSOT found; no stale active claim was retained.
- Retired / preserved surface decision: no source, contract, test, workflow, package or CLI surface was retired in this tranche. Existing script/materializer surfaces remain active only under private-inventory / machine-gate authority; no compatibility alias, fallback route, facade, wrapper or public entry was added.
- Verification boundary: docs-only lane. Run `git diff --check`, conflict-marker scan over README/docs/agent support READMEs, focused active-plan stale scan for script-to-pack wording, and OPL Doc doctor before commit. Source-purity tests are not required unless source/contracts/tests change; future script edits must run `tests/source-purity.test.ts`.
- Remaining OMA scope: registry/App live consumption, additional target cohorts, real source patch-loop, independent reviewer direct evidence, target-agent handoff convergence and actual script-to-pack source reductions remain open under the active gap plan.
