...
| direct graph / no-`--domain-id` implicit fixture baseline path and `scripts/lib/stage-decomposition-runner.ts` | `build-agent-baseline` consumes OPL StageRun query readbacks through `scripts/lib/action-stage-route.ts`; incomplete ordered routes return typed continuation, while `stage-decomposition` plans files and `agent-skeleton-build` owns file bodies. | Do not restore implicit graph production, direct script-authored stage graph fallback, OMA-owned fixture/live runner, typed-closeout CLI bypass, local attempt receipt writer, or compatibility materialization path. |
