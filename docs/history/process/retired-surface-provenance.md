...
| direct graph / no-`--domain-id` implicit fixture baseline path, `scripts/lib/stage-decomposition-runner.ts`, and `scripts/lib/action-stage-route.ts` | OPL hosted `stage_binding` owns StageRun/query/route transport; OMA keeps only `stage-decomposition` and `agent-skeleton-build` domain artifact semantics plus request/receipt consumption. | Do not restore implicit graph production, local StageRun readback evaluation, direct script-authored stage graph fallback, OMA-owned fixture/live runner, typed-closeout CLI bypass, local attempt receipt writer, or compatibility materialization path. |
...
| legacy repo-local Codex plugin / Skill wrapper, installer, sidecar, status shell, workbench shell, lifecycle placeholder, registry owner, App shell | OPL Framework/App own generated/hosted interfaces, registry/discovery, workbench shell, work-order execute/absorb/cleanup, queue, attempt ledger, and promotion gates. OMA's current `plugins/opl-meta-agent/` is only a repo-local full-copy carrier for `agent/primary_skill/SKILL.md`. | Do not recreate repo-owned generic CLI/MCP/Skill/product-entry wrapper, sidecar/status/workbench, scheduler, queue, attempt ledger, registry/App shell, or promotion engine. |
| `execute-external-work-order` repo-local CLI/action/schema/delegation envelope | `opl work-order execute` directly owns admission, execution, worktree lifecycle, receipt validation, absorb/cleanup and owner-closeout readback. OMA emits only developer work-order semantic requests. | Do not restore an OMA execution wrapper, OPL subprocess delegation, output writer, duplicate receipt validator, generated action, package script, or compatibility alias. |
