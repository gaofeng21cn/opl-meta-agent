...
- `opl-meta-agent` owns as agent-building professional skills：`agent/skills/oma-*.md` 下的 Codex-style 方法技能。这些技能把 intent architecture、external pattern research、stage pack architecture、Agent Lab suite design、takeover review、work-order authoring、agent-evolution diagnosis / patch playbook 和 trajectory learning analysis 从 stage prompt / domain skill prose 中拆出；它们仍只是 domain skill pack 的一部分，不是 runtime wrapper、target artifact、owner receipt、typed blocker 或 promotion gate state。
