...
OMA 侧执行入口保持薄委托：`execute:external-work-order` 读取已经生成的 developer patch work order，只校验 refs-only shape 与 authority boundary，然后调用 OPL Framework 的 `opl agent-lab execute-work-order --work-order <path> --json`。target worktree lifecycle、generic runner、queue、attempt ledger、absorb、cleanup、post-absorb owner closeout hook invocation、execution receipt 与 re-evaluation orchestration 均由 OPL Agent Lab / target owner 控制面持有；OMA 不在本仓实现第二套执行器、worktree 管理或 owner receipt writer。
