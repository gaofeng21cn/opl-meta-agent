...
OMA 的 profile 消费模式是三态：`builtin_profile`、`hybrid`、`source_derived_design`。`builtin_profile` 和 `hybrid` 消费 OPL `profiles select/inspect` 的 selected profile refs 与 requirements 作为 OPL conformance 下限；`source_derived_design` 用在无内置 profile trigger 命中但用户提供论文/PDF/repo/产品案例等 reference design 时，参考来源是设计起点，profile/catalog/template 不能替代设计来源。OMA 必须保留 OPL selector 的 source-derived route receipt、source refs、pattern packet refs、transferable pattern requirements 和 capability plan requirements，并先把论文/参考系统里的设计思路提炼成 `ReferenceDesignPacket`，再映射成 `TransferMap`，落成 `AgentPackPlan`，最后生成 `BuildReceipt` 和 target agent stage pack。`ReferenceDesignPacket` 必须包含 pattern packet 或 pattern note 派生出的 transferable patterns 和 extractable design aspects；只有 raw source ref 不等于已消费参考设计。内置 profile/catalog/template 只作为 lower-bound guardrail；OMA 不能因 catalog 缺模板而停止，也不能导入外部 runtime、私有数据、target truth、domain verdict 或 promotion authority。每个 source-derived stage requirement 必须声明来自参考设计的 `source_pattern_ref`/`stage_pattern_source_refs`，或明确标为 `target_only_requirement`；target-only owner gate 不能替代当前 stage 的 source-pattern 消费证明。`BuildReceipt` 必须把 source-derived stage refs、target-only requirements、rejected source patterns、forbidden claims 和 refs-only authority boundary 留在 OPL 可读 surface 中。
...
OMA 生成 target agent 时只提供 agent-building 语义，不维护私有 repo 目录标准。目标 repo 的物理骨架必须来自 OPL Framework 的 standard agent scaffold；scaffold 不提供目标 agent 设计来源。OMA 的 stage-decomposition、candidate package 和 developer work order 只能填充 domain-owned pack refs、stage refs、artifact morphology refs、owner route refs 和 closeout refs。
