...
外部 reference design 的机器边界只有一条：OPL 按 `opl.reference_design_pattern_packet.v1` 交付 body-free refs envelope，OMA 校验 envelope 后只在 packet 同目录安全解引用其 JSON-pointer semantic refs，再物化 OMA-owned content-rich `ReferenceDesignPacket`。OMA 不定义第二套 external packet ABI。`ReferenceDesignPacket` 保留真实 transferable steps、source anchors、`source_derived|internal_synthesis` provenance、适用/不可迁移约束与 user/seed dispositions；`TransferMap` 按 canonical pattern/step mapping fields 迁移；`AgentPackPlan` 为每个 workflow step 生成独立稳定 target stage。raw PDF/source ref、短 note、opaque packet、越界 pointer、缺 semantic refs 或 source-material mismatch 都在 profile/scaffold 物化前 fail closed 到 typed blocker。专家工作流 catalog 只作为 content-rich seed fallback/secondary，不是模板库；用户 packet 始终是 design origin。
...
- `opl-meta-agent` owns：agent-building semantics、intent brief、research brief、stage decomposition、candidate Agent Pack policy、AgentBuildReceipt、declarative Agent Lab suite specs、Foundry evaluation work-order candidate、external-result judgment 和 mechanism candidate 记录。
...
`contracts/stage_control_plane.json` 仅保持 existing consumer compatibility aggregate path；它及其 source/parts/bundle-manifest 继续由 `npm run stage-control:write` / `check` 防漂移，但不再是 OPL Pack stage compiler source。新增或修改现役 stage graph 必须先改 `agent/stages/manifest.json` 与被引用的 domain pack 文件；旧 aggregate 的维护只服务尚未切换的本仓测试/consumer，不能覆盖 manifest 或成为第二真相源。`contracts/source_structure_policy.json` 和 `scripts/check-source-structure.ts` 提供 family-level source-structure lane；默认 `structure` / `source-structure` 是 advisory，显式 `structure:strict` / `source-structure:strict` 才作为硬失败；旧 `line-budget` aliases 已退役。
...
1. `build-agent-baseline` 只消费 OPL StageRun query readbacks，并按 action catalog 的 ordered `stage_route` 校验同域 identity、exact attempt ref、accepted typed closeout、canonical completed outcome、无 conflict 和不可跳步。route 未闭合时返回 typed continuation，不生成 receipt。OPL ledger 只运输 closeout identity 和 refs；需要 domain payload 时，closeout 使用 canonical `domain_output={surface_kind=domain_owned_stage_output_ref, version=domain-owned-stage-output-ref.v1, domain_id, output_ref}`，且 `output_ref` 同时存在于 closeout refs。OMA 只接受 attempt `workspace_root` 内的 `file://` ref，并用同 ref 的 `closeout_ref_metadata` SHA-256 防 bytes drift、path traversal 与 symlink escape，再复核 stage/closeout identity。`stage-decomposition` payload 只持有目标 stage graph、action refs、文件物化计划、independent gate policy、quality gate declaration，以及 source-derived / research-driven 路线的 `StageDecompositionSubpacketSet`；`agent-skeleton-build` payload 独立持有与计划一一对应的文件正文。脚本严格校验两者并只对可机械推导的 subpacket 投影做一次有界 repair，随后调用 OPL `agents scaffold` 校验目标 agent package。OMA 不启动私有 stage attempt，也不保留 fixture/live runner、隐式 default graph 或兼容 fallback。
2. `eval-suite-build`：写入 declarative `agent-lab-suite-seed.json`，只包含 task manifest、recovery probe spec、scorecard spec、improvement candidate seed、promotion-gate request 和 refs，不包含 observation/pass/fail/gate status。
3. `baseline-run`：把完整 target identity、suite/task identity 与 canonical source/reviewer/candidate provenance 绑定到 `foundry-lab-work-order.json`；producer 到此只声明 ready for OPL Foundry Lab evaluation。
...
`target-agent-takeover` 读取目标 agent repo/package 的 descriptor/contracts 与 independent reviewer evidence，生成 declarative takeover suite seed、canonical Foundry evaluation work order、gated self-evolution candidate ref 和 mechanism candidate ref。它不调用 Agent Lab runner，不写 suite result、takeover/owner receipt、learning ledger 或 promotion result。

`improve-from-external-agent-lab-suite` 必须同时读取 declarative suite、显式 OPL Foundry Lab suite result 和 AI reviewer evaluation。reviewer critique、suggestions、source/direct evidence 与 provenance 进入 target capability improvement candidate 和 developer patch work order；可识别 suggestion 只在 target `contracts/capability_map.json` 声明 matching improvement token，或 target-owned production acceptance 显式声明 change-ref policy 时，映射到 `patch_traceability_matrix`。缺少 suite result、Foundry execution receipt、quality floor 或 target-owned patch policy 时，OMA 只返回 missing fields 与 expected typed-blocker ref，不物化 target-domain blocker body，也不生成 executable work order。

`agent:evidence` 读取目标 agent 的 standard production evidence handoff，只生成 declarative production-evidence suite seed、Foundry evaluation work order 和 proposal-only candidate refs。它不生成 Agent Lab run result、owner receipt、learning/scaleout ledger、promotion result 或 target-domain typed-blocker body。MAS/MAG 等目标 agent 只通过 domain refs 出现在 target identity 与 owner route 中；命令、suite kind、输出文件和 OMA surface kind 保持 target-agent generic。
