...
`opl-meta-agent` 采用轻量 OPL-compatible repo 文档形态：root `README*` 和 `docs/` 核心文档持有当前入口、项目事实、架构、硬约束、决策、目标态、active gap plan 与私有实现迁移台账。`agent/*/README.md` 是 domain pack 支撑索引，不作为 machine-required pack path；机器 pack 真相由 `contracts/pack_compiler_input.json`、`agent/stages/manifest.json`、被 manifest 引用的真实 pack 文件和 tests 验证。`contracts/stage_control_plane.json` 只保留旧 consumer compatibility aggregate。
