...
`opl-meta-agent` 采用轻量 OPL-compatible repo 文档形态：root `README*` 和 `docs/` 核心文档持有当前入口、项目事实、架构、硬约束、决策、目标态、active gap plan 与私有实现迁移台账。`agent/README.md` 是唯一顶层 pack 索引；`agent/professional_skills/README.md` 只保留 Codex professional skill discovery。两者都不是 machine-required pack path；机器 pack 真相由 `contracts/pack_compiler_input.json`、`agent/stages/manifest.json`、被 manifest 引用的真实 pack 文件和 tests 验证。Hosted `family_stage_control_plane` 由 OPL Framework 编译和持有。

当前 single Active Truth owner 是 `docs/active/opl-meta-agent-ideal-state-gap-plan.md`。North-star 目标态只读 `docs/references/opl-meta-agent-ideal-state.md`；OPL family Foundry Agent OS target delta 的机器 owner 是 `contracts/foundry-agent-os-domain-kernel-manifest.json`，人读 support owner 是 `docs/active/foundry-agent-os-target-delta.md`；私有实现、script hygiene、default-caller tail 和 OPL 上收边界只读 `docs/active/opl-private-implementation-migration-inventory.md`；核心当前事实继续回到 `docs/project.md`、`docs/status.md`、`docs/architecture.md`、`docs/invariants.md` 和 `docs/decisions.md`。
...
| `docs/active/foundry-agent-os-target-delta.md` | OMA 对 OPL family Foundry Agent OS 的 target delta 人读支撑 | `active_support` | 机器 owner 是 `contracts/foundry-agent-os-domain-kernel-manifest.json`；不声明 target-agent ready、App live rendering、human approval、default promotion 或 production ready。 |
...
| `docs/history/README.md` | history archive entry | `historical_archive_index` | 只导航历史目录；不作为当前 readiness、production ready、default promotion 或 owner receipt 证据。 |
...
| `agent/README.md`, `agent/professional_skills/README.md` | 顶层 pack 索引与 professional skill discovery | `active_support` | 真实 pack required paths 由 contracts/tests 验证；README 不作为 machine-required pack path。 |
...
| Agent pack support indexes | `agent/README.md`, `agent/professional_skills/README.md` |

## Rules

- `README*` 与 `docs/**` 是人读材料；机器消费者读取 contracts、source、tests、CLI/API 行为或语义化 refs。
- Foundry Agent OS target delta 的机器合同入口是 `contracts/foundry-agent-os-domain-kernel-manifest.json`；`docs/active/foundry-agent-os-target-delta.md` 只解释 owner split、OPL upcollect surfaces、OMA retained authority kernel 和 `current_owner_delta` 默认读根。
...
- `contracts/functional_privatization_audit.json` 是唯一 source-shape boundary；不能授权 target domain ready、App live rendering、owner receipt body、artifact readiness、production ready、quality verdict 或 default promotion。
- `contracts/pack_compiler_input.json` keeps `agent/` as the canonical semantic pack root and lists only non-README pack files in `required_domain_pack_paths`; its quality-gate source points to `agent/stages/manifest.json#/stages/*/quality_gate_refs`, not a README.

## Reopen Conditions

重新打开本文治理时，必须同时说明影响的唯一 owner doc 和验证入口：

- root `README*`、`docs/**/*.md`、`agent/README.md` 或 professional skill discovery inventory 变化。
- `contracts/foundry-agent-os-domain-kernel-manifest.json` 与 `docs/active/foundry-agent-os-target-delta.md` 的 owner split、default read root、upcollect surfaces 或 non-claims 不一致。
- README 被加入 machine-required pack paths，或 contracts/tests 与 README 边界不一致。
- active truth wording 新增 readiness、production、quality、App live、owner receipt 或 default promotion claim。
- `contracts/functional_privatization_audit.json`、`runtime/authority_functions/**` 或 source-purity tests 与本文 source-shape / evidence-tail 口径冲突。
...
The process history index is topic-level only: it records SSOT owners, compressed provenance groups, coverage summary, remaining evidence tails and next write scope. It must not grow back into dated closeout files, branch/worktree logs, proof transcripts, or coverage-ledger name lists. The current OMA portfolio recheck covers the complete tracked human-doc inventory; future records reopen this map only when inventory, machine boundaries, active truth claims, or retired-surface evidence changes.
