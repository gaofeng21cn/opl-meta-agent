...
| `docs/history/**` | process ledger、retired/provenance 和 no-resurrection 历史归档 | 不作为当前 readiness、production ready、default promotion 或 owner receipt 证据 |
...
Last semantic refresh: `2026-05-30T05:36:23+0800`.
...
| History / provenance | `docs/history/README.md`, `docs/history/process/README.md`, `docs/history/process/2026-05-30-oma-private-inventory-process-ledger.md` |
...
- The current exact inventory has one active truth owner, one ideal-state reference, one private implementation inventory, one docs lifecycle owner, one docs entry index, one history archive index, and five domain-pack support indexes. No duplicate active plan, stale current-truth path, or unsupported support README path is open.
- This file keeps only the current inventory, stale-claim guards, and reopening conditions. New coverage tranche evidence, doctor transcript, branch/worktree closeout, proof snapshot, or repeated no-drift packet belongs under `docs/history/process/`; durable conclusions fold back into the active gap plan, private implementation inventory, core docs, contracts, source, or tests.
...
- The obsolete `bootstrap:sample` npm alias, no-`--domain-id` implicit fixture smoke, no-closeout implicit fixture graph, `takeover:test --fixture` alias, and old direct graph compatibility wording are retired from package scripts, generated action command refs, active README/status wording, invariants, decisions, and build-agent-baseline materialization. `build-agent-baseline` is now the only public npm action entry for baseline generation, requires an explicit target `--domain-id`, and may use `--stage-runner fixture` only when `--stage-decomposition-closeout <path>` supplies an explicit proof/test typed closeout for deterministic validation.
...
- none. The current OMA human-doc inventory has legitimate long-term roles as public entry, docs index, current truth, active plan, active inventory, target-state reference, docs lifecycle ledger, history/provenance archive, or domain-pack support index. This tranche rewrote stale process-log wording in existing active docs and introduced a history archive instead of retiring a document path.
...
- Any future doc sentence that calls the stage-decomposition `fixture` runner a compatibility route, fallback materialization path, implicit graph source, direct script graph authority, public baseline path, or retained facade is stale pollution. It is only proof/test closeout input for deterministic validation of the same strict materializer, and it must carry an explicit typed closeout path.
...
### Historical Coverage Foldback

Dated coverage tranches and folded process history that previously lived here are archived in [OMA docs portfolio coverage ledger archive](./history/process/2026-06-02-oma-docs-portfolio-coverage-ledger-archive.md).

This active governance file now keeps only the current OMA coverage state, directory roles, stale-claim guards, and reopening conditions. Do not append future dated process logs, branch/worktree closeout, proof snapshots, or repeated no-drift packets here. New dated coverage belongs under `docs/history/process/`; durable conclusions must be folded back into the active gap plan, private implementation inventory, core docs, contracts, source, or tests.
