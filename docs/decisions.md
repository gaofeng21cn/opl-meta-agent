...
- 决策：OMA stage admission 采用 Progress-First 修正口径。validator、schema 或 materializer 发现的 format、projection、ref、stage input、`requires` 或 expected receipt 缺口，如果可由有效设计对象、stage graph 和 authority refs 机械推导，应在同一 stage materialization/admission 内有界修正并继续推进；不能直接变成 terminal blocker，也不能拆成新的 runtime stage 或私有控制面。核心语义对象、source/evidence、owner answer 或设计内容缺失时物化质量债、route-back input 或 no-output diagnostic，并允许 Codex advance、skip、repeat、reverse 或 route-back；这些缺口只关闭 admission/delivery/promotion/ready claim。authority 越权、forbidden target truth 写入、安全/权限、不可逆动作、wrong-target identity/currentness 或显式 human decision 才 fail closed。
...
- 决策：repo-tracked 和新生成的 Agent Package manifest 在未发布时保持 source-only，不预填 OCI `latest`、假 digest 或 non-live distribution proof。只有真实发布/registry 输入才携带 `distribution_payload`，并由 OPL Packages 严格校验 immutable ref 与 SHA-256；OMA 的公共生命周期入口固定为 `opl packages install|update|uninstall oma`，不保留 `opl connect` 私有安装语义。
- 决策：OPL external reference-design handoff 只采用 canonical `opl.reference_design_pattern_packet.v1` refs-only envelope；OMA 不复制其 required fields、不定义第二套 external packet ABI。OMA 校验 envelope/authority/non-claims，限制 semantic JSON-pointer 解引用在 packet 同目录，然后把被引用的 summary、transferable patterns、non-transferable constraints 和 authority notes 归一为 OMA-owned `ReferenceDesignPacket`。raw source、opaque packet 或缺 semantic refs 作为可消费 intake/extraction artifact 与质量债继续推进；它们只关闭 source-derived design admission/materialization claim。越界 pointer、不可读 bytes、authority/identity/currentness 冲突或 source-material identity mismatch 才写 hard-stop typed blocker。
...
- 决策：OMA 的 `family-action-catalog.v2` 不再携带 `source_command`、`handler_id` 或私有 command template。四个 domain-semantic action 只通过 `stage_binding` 进入 OPL-hosted StageRun；`generate-mechanism-patch-proposal` 通过 `handler_ref` 精确绑定 `contracts/domain_handler_registry.json` 中唯一 handler。`candidate_agent_package_builder` 已退出 minimal authority functions，candidate package 语义回到 declarative pack；唯一保留的 minimal authority function 是 `mechanism_patch_proposal_authorizer`。
...
- 决策：baseline production action 只通过 `family-action-catalog.v2` 的 `stage_binding` 消费 OPL hosted StageRun。Framework 持有 attempt/query/readback、identity/currentness、route recommendation 校验和 transition materialization；OMA 不保留 local route evaluator、raw StageRun readback 参数、current pointer 或 transition authority。OMA helper 只消费 `stage-decomposition` 与 `agent-skeleton-build` 的 domain closeout packet ref，以及 OPL-produced materialization/validation proof receipt；旧 direct script-authored graph、OMA-owned fixture/live runner、`standard-agent-action-stage-run` 兼容导出和 typed closeout CLI 旁路均无 compatibility path。
...
- 决策：source-derived / research-driven typed closeout 必须包含 `StageDecompositionSubpacketSet` v2。`design_object_packets` 是按 identity/ref 校验的无序集合，重排合法；`dependency_edges` 只固定 source/safe packet 先于 supported claim、design admission 先于 materialization、materialized bytes 先于 build receipt。TransferMap、AgentPackPlan 与 morphology 不固定认知顺序。
...
- 决策：`build-agent-baseline`、`takeover:test` 和 `agent:evidence` 的 producer 终点是 AgentBuildReceipt、OMA-owned evaluation semantics 与 canonical `opl_work_order_materialization_request.v2`；OMA 不分配 work-order identity，不管理 lease/worktree/lifecycle，不本地编译或执行 suite，也不写 Agent Lab result、Foundry execution receipt、target owner receipt、learning/scaleout ledger、promotion result 或 target-domain typed-blocker body。OPL Foundry Lab 消费完整 target identity 和 canonical provenance，物化并执行 canonical work order。
