...
- 决策：baseline materialization 通过 OPL public `standard-agent-action-stage-run` 消费 StageRun context，但不要求 ordered/no-skip、accepted typed closeout 或 conflict-free quality packet。任意可读 raw/partial artifact 可作为 `completed_with_quality_debt` 输入；Codex 可跳过、重复、逆向或 route-back。Framework 只校验 declared stage 与 identity/currentness/path safety；旧 direct script-authored graph、OMA-owned fixture/live runner 和 typed closeout CLI 旁路均无 compatibility path。
