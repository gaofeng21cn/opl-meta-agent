...
- 影响：只生成 repo skeleton、只跑 `opl agents scaffold --validate`、只跑 `opl agents interfaces`、只记录 baseline pass、只消费 StageRun / Temporal provider completion、只给 canary/operator summary 或只记录 conformance pass，都不能称为 OMA 新建智能体完成。`contracts/action_catalog.json#actions/build-agent-baseline/new_agent_delivery_gate` 与 `scripts/build-agent-baseline.ts#assertNewAgentDeliveryGate` 是当前机器守门入口；实际 baseline payload 必须带 `new_agent_delivery_gate` readback，并包含 StageRun/Temporal refs-only consumption、`stage_completion_policy_ref`、`stage_closeout_packet_ref`、target owner receipt / typed blocker / human gate ref、no-forbidden target truth write proof 和 target-domain artifact morphology reviewer evidence。若 Agent Lab 或 reviewer evidence 暴露缺口，必须进入 owner-gated improvement loop、developer work order 或 typed blocker；若无需 source patch，也必须记录 no-patch coordination work order / receipt 和 re-evaluation refs。
