...
- 决策：external-suite self-evolution 的 source patch target 只能来自目标 agent `contracts/capability_map.json` 或目标 agent 显式 policy；generic owner-receipt / package / typed-blocker 关键词不能生成 patch target。命不中时返回 target-improvement typed blocker；passed suite 只能生成 no-patch coordination shape。
