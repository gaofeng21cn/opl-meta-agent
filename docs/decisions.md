...
- 决策：OPL StageRun ledger 只持有 normalized closeout metadata 与 refs，不复制 OMA domain payload。需要完整设计/文件 payload 的 stage 必须在 `closeout_ref_metadata` 中提供 `role=oma_stage_closeout_payload`、workspace-relative ref 和 SHA-256；OMA 只在 attempt `workspace_root` 内安全解引用，拒绝 bytes drift、路径穿越、symlink escape、stage mismatch 和 closeout mismatch。
