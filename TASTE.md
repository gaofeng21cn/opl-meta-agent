...
对 OMA foundry agent、conformance、agent surface 或 project delivery 任务，真实进度按可接力 agent delta、owner decision、human gate、accepted finding、artifact delta 或用户可用结果计算；runtime、queue、stage、read model、测试、docs、provider liveness 和平台修复只是支持证据。OMA/OPL 执行基座是首选路径，不是目标交付的前置条件：基座顺畅时走基座，基座卡住时，前端执行者继续推进合法的 foundry/agent 交付增量，并把暴露出的基座问题作为 side repair lane 记录或修复。只有继续动作会越权写 agent/domain authority surface、缺少必要 source/spec、或同一写集 ownership 冲突且无法隔离时，才暂停具体目标动作。repair lane 不能吞掉目标主线，除非用户目标本身就是平台修复。
